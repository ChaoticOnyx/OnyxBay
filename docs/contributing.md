...
### block() вместо range() для обработки турфов
Вместо `for(var/turf/T in range(N, src))` и подобного лучше написать `for(var/turf/T in block(locate(x-N, y-N, z), locate(x+N, y+N, z)))`
Функция block() возвращает ТОЛЬКО лист турфов и работает примерно в 16 раз быстрее, чем аналог с использованием range(). Если же вам нужно пройтись по всем турфам кроме центрального, просто вычитайте его из возвращаемого списка так `for(var/turf/T in (block(locate(x-N, y-N, z), locate(x+N, y+N, z)) - src))` (в случае если src является турф от которого вы всё считаете)

### Оператор #define
Любая функция несёт в себе небольшой оверхед, поэтому чтобы писать производительный код следует заменять любые "маленькие" функции на их аналоги в виде дефайнов, например:
```
/proc/give_sum(a, b)
	return a + b
```
Можно с легкостью заменить на `#define GIVE_SUM(a, b) (a + b)`

### Тонкости работы с switch()
Оператор switch() довольно прост в понимании и использовании, однако в некоторых ситуациях полезно знать особенности его поведения, например, рассмотрим такой код
```
var/variable = 1
switch(variable)
	if(1)
	...
	if(5 to 10)
		world << "THIRD CASE"
```
Казалось бы, всё довольно просто и в чате мы должны увидеть "FIRST CASE", однако увидим мы "SECOND CASE". При пересечении значений бьенд отдаёт предпочтение кейсам с областью значений, поэтому чтобы избежать таких неурядиц, область значений нужно задавать в каждом из пересекающихся кейсов. Вот так будет выглядеть работающий как нам надо код для случая выше:
```
var/variable = 1
switch(variable)
	if(1 to 1)
		world.log << "FIRST CASE"
	if(1 to 5)
		world.log << "SECOND CASE"
	if(5 to 10)
		world.log << "THIRD CASE"
```
