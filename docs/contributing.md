...
### Обращайте внимание на существующие макросы и используйте их везде, где это возможно
Пара примеров:
* Макросы проверки типа (`isliving`, `isghost`, `isatom`, ...)
* Макросы оформления текста SPAN_* (`SPAN_NOTICE`, `SPAN_WARNING`, `SPAN_DANGER`, ...)
* Вспомогательные функции логирования (`log_and_message_admins`, `log_debug`, ...)
