...
/datum/controller/subsystem/statpanels/proc/return_object_images(client/load_from)
	...
	for(var/atom/turf_item as anything in obj_window.atoms_to_show)
		...
		obj_window.register_signal(turf_item, SIGNAL_QDELETING, TYPE_PROC_REF(/datum/object_window_info,viewing_atom_deleted)) // we reset cache if anything in it gets deleted
	return turf_items
...
/datum/object_window_info/proc/start_turf_tracking()
	...
	var/static/list/connections = list(
		SIGNAL_MOVABLE_MOVED = PROC_REF(on_mob_move),
		SIGNAL_LOGGED_OUT = PROC_REF(on_mob_logout),
	)
...
/datum/object_window_info/proc/stop_turf_tracking()
	qdel(get_components(/datum/component/connect_mob_behalf))
