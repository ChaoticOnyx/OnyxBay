...
/datum/spatial_grid_cell/New(cell_x, cell_y, cell_z)
	...
	if(length(dummy_list))
		dummy_list.Cut()
		util_crash_with("SSspatial_grid.dummy_list had something inserted into it at some point! this is a problem as it is supposed to stay empty")
...
SUBSYSTEM_DEF(spatial_grid)
	can_fire = FALSE
	init_order = SS_INIT_SPATIAL_GRID
	name = "Spatial Grid"
...
/datum/controller/subsystem/spatial_grid/Initialize()
	...
	for(var/channel_type in waiting_to_add_by_type)
		for(var/atom/movable/movable as anything in waiting_to_add_by_type[channel_type])
			...
			unregister_signal(movable, SIGNAL_QDELETING)
...
/datum/controller/subsystem/spatial_grid/proc/enter_pre_init_queue(atom/movable/waiting_movable, type)
	register_signal(waiting_movable, SIGNAL_QDELETING, nameof(.proc/queued_item_deleted), override = TRUE)
...
/datum/controller/subsystem/spatial_grid/proc/remove_from_pre_init_queue(atom/movable/movable_to_remove, exclusive_type)
	if(exclusive_type)
		...
		if(!waiting_movable_is_in_other_queues)
			unregister_signal(movable_to_remove, SIGNAL_QDELETING)
...
/datum/controller/subsystem/spatial_grid/proc/after_world_bounds_expanded(datum/controller/subsystem/fucking_dcs, has_expanded_world_maxx, has_expanded_world_maxy)
	SIGNAL_HANDLER
...
/datum/controller/subsystem/spatial_grid/proc/remove_single_type(atom/movable/old_target, turf/target_turf, exclusive_type)
	if(!target_turf || !exclusive_type || !old_target.spatial_grid_key)
		util_crash_with("/datum/controller/subsystem/spatial_grid/proc/remove_single_type() was given null arguments or an old_target that doesn't use the spatial grid!")
		return FALSE
...
/datum/controller/subsystem/spatial_grid/proc/untracked_movable_error(atom/movable/movable_to_check)
	...
	if(length(containing_cells) > 1 || (length(containing_cells) == 1 && loc_cell && containing_cells[1] != loc_cell && containing_cells[1] != null))
		 ...
		 */
		util_crash_with("[movable_to_check.type] instance, [location_string], [error_explanation] [error_data].")
...
/datum/controller/subsystem/spatial_grid/proc/assign_oranges_ears(list/atoms_that_need_ears)
	...
	if(input_length > number_of_oranges_ears)
		util_crash_with("somehow, for some reason, more than the preset generated number of oranges ears was requested. thats fucking [number_of_oranges_ears]. this is not good that should literally never happen")
...
/atom/proc/find_grid_statistics_for_z_level(insert_clients = 0)
	...
	if(insert_clients)
		var/list/turfs
		var/datum/space_level/level = GLOB.using_map[z]
		if(level.has_trait(ZTRAIT_STATION))
