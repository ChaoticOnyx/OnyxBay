...
/datum/controller/subsystem/holomaps/proc/generate_holo_map()
	...
	var/turf/center = locate(world.maxx/2, world.maxy/2, GLOB.using_map.level_has_trait(1, ZTRAIT_STATION))
	if(!center)
		return
	var/list/turf/turfs = RANGE_TURFS(world.maxx/2, center)
	for(var/turf/T as anything in turfs)
		if (isfloor(T) || istype(T, /turf/unsimulated/floor) || istype(T, /turf/simulated/floor/shuttle))
			holomap.DrawBox(HOLOMAP_WALKABLE_TILE, T.x, T.y)
		if(iswall(T) || istype(T, /turf/unsimulated/wall) || locate(/obj/structure/grille) in T)
			holomap.DrawBox(HOLOMAP_CONCRETE_TILE, T.x, T.y)
		if(locate(/obj/structure/window) in T)
			holomap.DrawBox(HOLOMAP_GLASS_TILE, T.x, T.y)
	return holomap

#undef HOLOMAP_WALKABLE_TILE
#undef HOLOMAP_CONCRETE_TILE
#undef HOLOMAP_GLASS_TILE
...
/datum/controller/subsystem/holomaps/proc/process_holomap_markers()
	for(var/freq in SSholomaps.holochips)
		for(var/obj/item/holochip/HC in SSholomaps.holochips[freq])
			...
			if(!marker_location)
				util_crash_with("[HC.holder] | [HC.holder.loc] | [HC.frequency] without turf.")
				continue
			if(!GLOB.using_map.level_has_trait(marker_location.z, ZTRAIT_STATION))
				continue
	...
	for(var/obj/machinery/computer/shuttle in holomap_landmarks)
		...
		if(!marker_location)
			util_crash_with("[shuttle.type] without turf.")
			continue
		if(!GLOB.using_map.level_has_trait(marker_location.z, ZTRAIT_STATION))
			continue
		/*
