...
/datum/controller/subsystem/processing/physics_processing/proc/AddToLevel(datum/component/physics2d/newP, target_z)
	var/z_str = "[target_z]"
	if(!quadtrees[z_str])
		quadtrees[z_str] = new /datum/quadtree(null, 0, world.maxx * 32 / 2, world.maxy * 32 / 2, world.maxx * 32, world.maxy * 32) // create new tree
...
// Try to optimize overrides as much as possible
/// Can we collide with this atom?
/atom/movable/proc/physics_collide(atom/movable/A)
	return TRUE
...
/datum/component/physics2d/proc/update_z()
	if(holder.z != last_registered_z) //Z changed? Update this unit's processing chunk.
		if(!holder.z) // Something terrible has happened. Kill ourselves to prevent runtime spam
			qdel(src)
			message_admins("WARNING: [holder] has been moved out of bounds at [admin_jump_link(holder.loc)]. Deleting physics component.")
