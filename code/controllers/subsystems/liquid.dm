...
/datum/controller/subsystem/liquids/proc/get_immutable(type)
	if(!singleton_immutables[type])
		var/atom/movable/liquid_turf/immutable/new_one = new type()
...
/datum/controller/subsystem/liquids/proc/mark_active_turf(turf/T)
	if(can_fire && !active_turfs[T])
		active_turfs[T] = TRUE
		if(T.lgroup)
			T.lgroup.amount_of_active_turfs++

/datum/controller/subsystem/liquids/proc/remove_turf(turf/T)
	if(active_turfs[T])
		active_turfs -= T
		if(T.lgroup)
			T.lgroup.amount_of_active_turfs--
