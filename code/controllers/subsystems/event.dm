...
/datum/controller/subsystem/event/proc/GetInteractWindow()
	...
	html += "<A align='right' href='?src=\ref[src];pause_all=[!config.random_events.allow_random_events]'>Pause All - [config.random_events.allow_random_events ? "Pause" : "Resume"]</A>"
...
/datum/controller/subsystem/event/Topic(href, href_list)
	...
	if(href_list["toggle_report"])
		...
		config.random_events.allow_random_events = text2num(href_list["pause_all"])
		log_and_message_admins("has [config.random_events.allow_random_events ? "resumed" : "paused"] countdown for all events.")
