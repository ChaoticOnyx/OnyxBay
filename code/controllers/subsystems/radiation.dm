...
/datum/controller/subsystem/radiation/fire(resumed = FALSE)
	...
	while(current_sources.len)
		...
		if(QDELETED(S))
			...
			S.update_rad_power(S.rad_power - config.radiation.radiation_decay_rate)
		if (MC_TICK_CHECK)
			return
	...
	for(var/value in sources)
		...
		while(origin != T)
			...
			if(origin.cached_rad_resistance)
				working = round((working / (origin.cached_rad_resistance * config.radiation.radiation_resistance_multiplier)), 0.1)
