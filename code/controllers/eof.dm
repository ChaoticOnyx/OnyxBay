...
/datum/controller/subsystem/ticker/proc/pick_eof_event()
	if(config.roundstart_events && prob(eof_drop_chance()))
		var/list/runnable_events = config.GetRunnableEoFModes()
		eof = config.pick_eof(pickweight(runnable_events))
		log_debug("EoF [eof.id] has been dropped!")
	return

/datum/controller/subsystem/ticker/proc/apply_eof_event()
	if(eof)
		eof.apply_event()
		eof.announce_event()
	return
...
/datum/round_event/lack_of_energy/apply_event()
	for(var/obj/machinery/power/supermatter/SM in GLOB.energy_sources)
		qdel(SM)
	for(var/obj/machinery/the_singularitygen/LORDSINGULO in GLOB.energy_sources)
		qdel(LORDSINGULO)
...
/datum/round_event/old_times/apply_event()
	for(var/atom/movable/lighting_overlay/LO in GLOB.lighting_overlays)
...
/datum/round_event/ghetto_medbay/apply_event()
	var/cur_y = num2text((text2num(time2text(world.timeofday, "YYYY"))+544))
	for(var/area/medical/M in GLOB.area_med)
		...
		for(var/obj/structure/closet/secure_closet/medical_wall/SCMW in M)
			qdel(SCMW)
		for(var/obj/item/stack/material/plasma/MP in M)
			qdel(MP)
		...
		for(var/obj/item/weapon/reagent_containers/spray/cleaner/SC in M)
			...
			SD.desc = "In [cur_y] someone need space cleaners?"
			qdel(SC)
...
/datum/round_event/clumpsy_dumbasses/apply_event()
	for(var/mob/living/carbon/human/H in GLOB.human_mob_list) //TODO: apply to latejoined players
		if(prob(69))
...
/datum/round_event/can_you_hear_me_major_tom/apply_event()
	for(var/obj/item/device/radio/headset/RH in GLOB.listening_objects)
		qdel(RH)
...
/datum/round_event/pussy_riot/apply_event()
	for(var/obj/item/weapon/melee/baton/B in GLOB.spec_sec_items)
		B.color = COLOR_LIGHT_PINK
	for(var/obj/item/weapon/handcuffs/H in GLOB.spec_sec_items)
		H.icon = 'icons/obj/pinkcuffs.dmi'
		H.icon_state = "pinkcuffs"
		H.update_icon()
	for(var/area/security/S in GLOB.area_brig)
