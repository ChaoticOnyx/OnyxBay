...
/proc/log_story(type, message, location)
	...
	if(!config.log.log_story || !GLOB.world_story_log)
		return
...
/proc/log_to_dd(text)
	to_world_log(text)
	if(config && config.log.log_world_output)
		log_debug("\[DD]: [text]")
...
/proc/log_roundend(text)
	log_generic("ROUNDEND", text, null, config.log.log_game)

/proc/log_admin(text, location, notify_admin)
	log_generic("ADMIN", text, location, config.log.log_admin, notify_admin, MESSAGE_TYPE_ADMINLOG)

/proc/log_debug(text, location, type = MESSAGE_TYPE_DEBUG)
	log_generic("DEBUG", SPAN("filter_debuglog", text), location, FALSE, TRUE, type)
	if(!config.log.log_debug || !GLOB.world_debug_log)
		return
	WRITE_FILE(GLOB.world_debug_log, "\[[time_stamp()]] DEBUG: [text][log_end]")

/proc/log_debug_verbose(text)
	if(!config.log.log_debug_verbose || !GLOB.world_debug_log)
		return
	WRITE_FILE(GLOB.world_debug_log, "\[[time_stamp()]] DEBUG VERBOSE: [text][log_end]")

/proc/log_game(text, location, notify_admin)
	log_generic("GAME", text, location, config.log.log_game, notify_admin)

/proc/log_vote(text)
	log_generic("VOTE", text, null, config.log.log_vote)

/proc/log_access(text, notify_admin)
	log_generic("ACCESS", text, null, config.log.log_access, notify_admin, MESSAGE_TYPE_ADMINLOG)

/proc/log_say(text)
	log_generic("SAY", text, null, config.log.log_say)
	log_story("SAY", text, null)

/proc/log_ooc(text)
	log_generic("OOC", text, null, config.log.log_ooc)
	log_story("OOC", text, null)

/proc/log_whisper(text)
	log_generic("WHISPER", text, null, config.log.log_whisper)
	log_story("WHISPER", text, null)

/proc/log_emote(text)
	log_generic("EMOTE", text, null, config.log.log_emote)
	log_story("EMOTE", text, null)

/proc/log_attack(text, location, notify_admin)
	log_generic("ATTACK", text, location, config.log.log_attack, notify_admin, MESSAGE_TYPE_ATTACKLOG)
	log_story("ATTACK", text, location)

/proc/log_adminsay(text)
	log_generic("ADMINSAY", text, null, config.log.log_adminchat, FALSE, MESSAGE_TYPE_ADMINLOG)

/proc/log_adminwarn(text, location, notify_admin)
	log_generic("ADMINWARN", text, location, config.log.log_adminwarn, notify_admin, MESSAGE_TYPE_ADMINLOG)

/proc/log_pda(text)
	log_generic("PDA", text, null, config.log.log_pda)
