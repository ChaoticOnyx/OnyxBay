...
/datum/beam/proc/Start()
	...
	register_signal(origin, SIGNAL_QDELETING, .proc/redrawing)
	register_signal(target, SIGNAL_QDELETING, .proc/redrawing)
...
/datum/beam/proc/redrawing(atom/movable/mover, atom/oldloc, new_loc)
	if(!QDELETED(origin) && !QDELETED(target) && get_dist(origin,target)<max_distance && origin.z == target.z)
		...
		INVOKE_ASYNC(src, nameof(.proc/Draw))
	else
		qdel(src)
...
/datum/beam/Destroy()
	QDEL_NULL_LIST(elements)
	QDEL_NULL(visuals)
	unregister_signal(origin, SIGNAL_MOVED, nameof(.proc/redrawing))
	unregister_signal(target, SIGNAL_MOVED, nameof(.proc/redrawing))
	unregister_signal(origin, SIGNAL_QDELETING, nameof(.proc/redrawing))
	unregister_signal(target, SIGNAL_QDELETING, nameof(.proc/redrawing))
	target = null
	origin = null
	return ..()
...
/atom/proc/Beam(atom/BeamTarget,icon_state="b_beam",icon='icons/effects/beam.dmi',time=INFINITY,maxdistance=INFINITY,beam_type=/obj/ebeam)
	...
	INVOKE_ASYNC(newbeam, nameof(/datum/beam/.proc/Start))
	return newbeam
