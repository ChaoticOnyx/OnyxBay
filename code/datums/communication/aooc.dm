...
/decl/communication_channel/aooc/do_communicate(client/C, message)
	...
	for(var/client/target in GLOB.clients)
		...
		else if(target.mob.mind?.special_role && (target.mob.mind.special_role == C.mob.mind.special_role || holder))
			var/display_name = C.key
			var/player_display = holder ? "[display_name]([holder.rank])" : display_name
