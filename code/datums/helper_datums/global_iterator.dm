...
/datum/global_iterator/New(list/arguments = null, autostart = 1)
	...
	// prevents garbage collection with tag != null
	if(forbid_garbage)
		tag = "\ref[src]"
...
/datum/global_iterator/proc/main()
	...
	while(src && control_switch)
		...
		// uhh, this is ugly. But I see no other way to terminate sleeping proc. Such disgrace.
...
/datum/global_iterator/proc/start(list/arguments = null)
	...
	if(arguments)
		if(!set_process_args(arguments))
			return 0
	// the main loop is sleeping, wait for it to terminate.
	if(!state_check())
		return
	control_switch = 1
	spawn()
...
/datum/global_iterator/proc/stop()
	if(!active())
		return
	control_switch = 0
	// report termination error but don't wait for state_check().
	spawn(-1)
		state_check()
	return 1
