...
GLOBAL_LIST_EMPTY(state_machines)

/// Global repository of all /datum/state
...
/datum/state_machine
	...
	var/datum/state/current_state = null // Acts both as a ref to the current state and holds which state it will default to on init.

/datum/state_machine/New(datum/_holder)
	..()
	if(!istype(_holder))
		CRASH("Non-datum holder supplied to [type] New().")
	else
...
/datum/state_machine/proc/set_state(new_state_type)
	...
	if(ispath(new_state_type))
		current_state = GLOB.fsm_states[new_state_type]
