...
/datum/event2/radiation_storm/get_conditions_description()
	. = "<em>Radiation Storm</em> should not be <em>running</em>.<br>"

/datum/event2/radiation_storm/check_conditions()
	. = SSevents.evars["radiation_storm_running"] != TRUE

/datum/event2/radiation_storm/get_mtth()
	. = ..()
	. -= (SSevents.triggers.roles_count["Medical"] * (22 MINUTES))
	. = max(1 HOUR, .)

/datum/event2/radiation_storm/on_fire()
	SSevents.evars["radiation_storm_running"] = TRUE
...
/datum/event2/radiation_storm/proc/enter_belt()
...
/datum/event2/radiation_storm/proc/exit_belt()
...
/datum/event2/radiation_storm/think()
	switch(state)
...
/datum/event2/radiation_storm/proc/end()
	SSevents.evars["radiation_storm_running"] = FALSE
	set_next_think(0)
	revoke_maint_all_access()
	QDEL_LIST(rad_sources)

/datum/event2/radiation_storm/proc/radiate()
	rad_sources = list()
...
/datum/event2/radiation_storm/syndicate
...
/datum/event2/radiation_storm/syndicate/think()
	return

/datum/event2/radiation_storm/syndicate/radiate()
	return
