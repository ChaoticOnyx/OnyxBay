...
/datum/uplink_item/deity/get_goods(obj/item/device/uplink/U, loc)
	if(!path) //Prolly a feat or activator.
		return 1
	return new path(null)

/datum/uplink_item/deity/boon/buy(obj/item/device/uplink/U, mob/living/deity/user)
	. = ..()
	if(.)
		user.set_boon(.)
...
/datum/uplink_item/deity/feat/buy(obj/item/device/uplink/U, mob/living/deity/user)
	. = ..()
	if(.)
		user.feats |= src.name

/datum/uplink_item/deity/feat/phenomena/buy(obj/item/device/uplink/U, mob/living/deity/user)
	. = ..()
	if(.)
...
/datum/uplink_item/deity/leveled/proc/get_level(mob/living/deity/user)
...
/datum/uplink_item/deity/leveled/buy(obj/item/device/uplink/U, mob/living/deity/user)
	. = ..()
	if(.)
		if(!user.feats[src.name])
