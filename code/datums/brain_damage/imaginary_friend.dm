...
/datum/brain_trauma/special/imaginary_friend/proc/get_ghost()
	var/datum/callback/to_call = CALLBACK(src, .proc/add_friend)
...
/mob/camera/imaginary_friend/send_speech(message, range = 7, obj/source = src, bubble_type = bubble_icon, list/spans = list(), datum/language/message_language = null, list/message_mods = list(), forced = null)
	...
	INVOKE_ASYNC(GLOBAL_PROC, .proc/flick_overlay_global, bubble, speech_bubble_recipients, 3 SECONDS)
...
/datum/emote/imaginary_friend/custom/run_emote(mob/user, params, type_override = null, intentional = FALSE)
	...
	if(is_banned_from(user.ckey, "Emote"))
		to_chat(user, SPAN_WARNING(SPAN_BOLD("You cannot send custom emotes (banned).")))
		return FALSE
	else if(QDELETED(user))
		return FALSE
	else if(user.client && user.client.prefs.muted & MUTE_IC)
		to_chat(user, SPAN_WARNING(SPAN_BOLD("You cannot send IC messages (muted).")))
		return FALSE
	else if(!params)
...
/mob/camera/imaginary_friend/point_at(atom/pointed_atom)
	...
	INVOKE_ASYNC(GLOBAL_PROC, .proc/flick_overlay_global, visual, group_clients(), 2.5 SECONDS)
