...
#define ATOM_FLAG_CHECKS_BORDER          1<<1 // If a dense atom (potentially) only blocks movements from a given direction, i.e. window panes
#define ATOM_FLAG_CLIMBABLE              1<<2 // This object can be climbed on
#define ATOM_FLAG_NO_BLOOD               1<<3 // Used for items if they don't want to get a blood overlay.
#define ATOM_FLAG_NO_REACT               1<<4 // Reagents don't react inside this container.
...
#define ATOM_AWAITING_OVERLAY_UPDATE     1<<9 // SSoverlays must update this atom's overlays.
#define ATOM_FLAG_SILENTCONTAINER        1<<10 // Chemical reaction won't trigger bubbling sound
#define ATOM_FLAG_UNPUSHABLE             1<<11 // This atom can't be pushed when bumped
