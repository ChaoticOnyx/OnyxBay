...
/client/proc/bluespace_tech()
	...
	if(!isghost(mob))
		to_chat(src, SPAN("warning", "You must to be a ghost to use this!"))
		return
...
/mob/living/carbon/human/bluespace_tech/can_inject(mob/user, target_zone)
	to_chat(user, SPAN("danger", "The [src] disarms you before you can inject them."))
	user.drop_item()
	return 0
...
/mob/living/carbon/human/bluespace_tech/verb/antigrav()
	...
	if (fall_override)
		fall_override = FALSE
		to_chat(usr, SPAN("notice", "You will now fall normally."))
	else
		fall_override = TRUE
		to_chat(usr, SPAN("notice", "You will no longer fall."))
...
/mob/living/carbon/human/bluespace_tech/verb/bstwalk()
	...
	if(usr.HasMovementHandler(/datum/movement_handler/mob/incorporeal))
		...
		to_chat(usr, SPAN("notice", "You will no-longer phase through solid matter."))
	else
		usr.ReplaceMovementHandler(/datum/movement_handler/mob/incorporeal)
		to_chat(usr, SPAN("notice", "You will now phase through solid matter."))
...
/mob/living/carbon/human/bluespace_tech/verb/tgm()
	...
	to_chat(usr, SPAN("notice", "God mode is now [status_flags & GODMODE ? "enabled" : "disabled"]."))
...
/obj/item/weapon/storage/backpack/holding/bluespace_tech/attack_hand(mob/user)
	...
	if(!istype(user, /mob/living/carbon/human/bluespace_tech))
		to_chat(user, SPAN("warning", "Your hand seems to go right through the [src]. It's like it doesn't exist."))
		return
	return ..()
...
/obj/item/clothing/glasses/sunglasses/bluespace_tech/verb/toggle_xray(mode in list("X-Ray without Lighting", "X-Ray with Lighting", "Normal"))
	...
	to_chat(usr, SPAN("notice", "\The [src]'s vision mode is now <b>[mode]</b>."))
