...
/proc/admin_attack_log(mob/attacker, mob/victim, attacker_message, victim_message, admin_message)
	...
	if(attacker && victim)
		...
		log_story("ATTACK", "[key_name(attacker, hide_ckey = TRUE)] [admin_message] [key_name(victim, hide_ckey = TRUE)] (INTENT: [attacker? uppertext(attacker.a_intent) : "N/A"])")
	else if(attacker)
		full_admin_message = "[key_name(attacker)] [admin_message] (INTENT: [attacker? uppertext(attacker.a_intent) : "N/A"])"
		log_story("ATTACK", "[key_name(attacker, hide_ckey = TRUE)] [admin_message] (INTENT: [attacker? uppertext(attacker.a_intent) : "N/A"])")
	else
		full_admin_message = "[key_name(victim)] [admin_message]"
		log_story("ATTACK", "[key_name(victim, hide_ckey = TRUE)] [admin_message]")
