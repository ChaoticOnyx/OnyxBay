...
/datum/admin_secret_item/fun_secret/ghost_mode/execute(mob/user)
	...
	for(var/mob/M in GLOB.living_mob_list_)
		if(M.stat == CONSCIOUS && !(M in affected_mobs))
			...
			switch(rand(1,4))
				if(1)
					M.show_message(text(SPAN("notice", "You shudder as if cold..."), 1))
				if(2)
					M.show_message(text(SPAN("notice", "You feel something gliding across your back..."), 1))
				if(3)
					M.show_message(text(SPAN("notice", "Your eyes twitch, you feel like something you can't see is here..."), 1))
				if(4)
					M.show_message(text(SPAN("notice", "You notice something moving out of the corner of your eye, but nothing is there..."), 1))
	...
	for(var/mob/M in affected_mobs)
		M.show_message(text(SPAN("notice", "The chilling wind suddenly stops..."), 1))
	affected_mobs.Cut()
	affected_areas.Cut()
