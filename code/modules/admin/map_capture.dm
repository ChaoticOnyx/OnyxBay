/datum/admins/proc/capture_map_part(tx as null|num, ty as null|num, tz as null|num, range as null|num)
	...
	if(!check_rights(R_ADMIN|R_DEBUG|R_SERVER))
		to_chat(usr, "You are not allowed to use this command", confidential = TRUE)
		return

	if(isnull(tx) || isnull(ty) || isnull(tz) || isnull(range))
		to_chat(usr, "Capture Map Part, captures part of a map using camara like rendering.", confidential = TRUE)
		to_chat(usr, "Usage: Capture-Map-Part target_x_cord target_y_cord target_z_cord range", confidential = TRUE)
		to_chat(usr, "Target coordinates specify bottom left corner of the capture, range defines render distance to opposite corner.", confidential = TRUE)
		return

	if(range > 32 || range <= 0)
		to_chat(usr, "Capturing range is incorrect, it must be within 1-32.", confidential = TRUE)
		return
	...
	if(locate(tx,ty,tz))
		...
		to_chat(usr, "Saved capture in cache as [file_name].", confidential = TRUE)
		send_rsc(usr, cap, file_name)
	else
		to_chat(usr, "Target coordinates are incorrect.", confidential = TRUE)
...
/datum/admins/proc/capture_map_capture_next(currentz, currentx, currenty, ligths)
	if(locate(currentx, currenty, currentz))
		...
		if(locate(currentx, currenty, currentz))
			...
			to_chat(usr, "End of map, capture is done.", confidential = TRUE)
...
/datum/admins/proc/capture_map(tz as null|num)
	...
	if(isnull(tz))
		to_chat(usr, "Map Part, map using camara like rendering.", confidential = TRUE)
		to_chat(usr, "Usage: Capture-Map target_z_cord", confidential = TRUE)
		to_chat(usr, "Target Z coordinates define z level to capture.", confidential = TRUE)
		return

	if(!locate(1, 1, tz))
		to_chat(usr, "Target z-level is incorrect.", confidential = TRUE)
		return
