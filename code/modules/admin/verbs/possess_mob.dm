...
/client/proc/toggle_possess_mode(mob/living/M in range(view))
	...
	if(!isliving(M))
		return
	if(M.client)
		to_chat(src, SPAN("warning", "You can't toggle possess mode for mobs with client!"))
		return
	if(M.ckey && copytext(M.ckey, 1, 2) == "@")
		to_chat(src, SPAN("warning", "[M] is occupied with aghosted admin."))
		return
	if(M.stat == DEAD)
		to_chat(src, SPAN("warning", "[M] is dead. There's no point to toggle possess mode!"))
		return
	...
	for(var/mob/observer/ghost/G in GLOB.player_list)
		if(G.client)
			if(jobban_isbanned(G, "Animal"))
				continue
			to_chat(G, SPAN("deadsay", "<b>[create_ghost_link(G, M, "(F)")] [capitalize(M.name)] is now available to possess! [possess_link(G, M)]</b>"))
