...
/datum/build_mode/advanced/Help()
	...
	to_chat(user, "<span class='notice'>Left Click                       = Create objects</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Right Click                      = Delete objects</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Left Click + Ctrl                = Capture object type</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Middle Click                     = Capture object type</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Right Click on Build Mode Button = Select object type</span>", confidential = TRUE)
...
/datum/build_mode/advanced/OnClick(atom/A, list/parameters)
	if(parameters["left"] && !parameters["ctrl"])
		if(ispath(build_type,/turf))
			...
			to_chat(user, "<span>Select a type to construct.</span>", confidential = TRUE)
...
/datum/build_mode/advanced/proc/SetBuildType(atom_type)
	...
	if(ispath(atom_type, /atom))
		...
		to_chat(user, "<span class='notice'>Will now construct instances of the type [atom_type].</span>", confidential = TRUE)
	else
		to_chat(user, "<span class='warning'>Cannot construct instances of type [atom_type].</span>", confidential = TRUE)
