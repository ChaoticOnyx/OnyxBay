...
/datum/build_mode/throw_at/Help()
	...
	to_chat(user, "<span class='notice'>Left Click on Movable Atom = Select object to be thrown</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Right Click on Atom        = Throw at the target</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>***********************************************************</span>", confidential = TRUE)
	...
	if(parameters["left"])
		...
		if(to_throw)
			if(!isturf(to_throw.loc))
				to_chat(user, "<span class='warning'>\The [to_throw] is currently not on a turf and cannot be thrown.</span>", confidential = TRUE)
			else
				to_throw.throw_at(A, 10, 1)
				Log("Threw '[log_info_line(to_throw)]' at '[log_info_line(A)]'")
		else
			to_chat(user, "<span class='warning'>You have nothing selected to throw.</span>", confidential = TRUE)
