...
/datum/build_mode/relocate_to/Help()
	...
	to_chat(user, "<span class='notice'>Left Click on Movable Atom = Select object to be relocated</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>Right Click on Turf        = Destination to be relocated to</span>", confidential = TRUE)
	to_chat(user, "<span class='notice'>***********************************************************</span>", confidential = TRUE)
	...
	if(parameters["left"])
		...
		if(to_relocate)
			...
			if(destination_turf)
				...
				to_chat(user, "<span class='warning'>Unable to locate destination turf.</span>", confidential = TRUE)
		else
			to_chat(user, "<span class='warning'>You have nothing selected to relocate.</span>", confidential = TRUE)
...
/datum/build_mode/relocate_to/proc/SetRelocator(new_relocator)
	...
	to_chat(user, "<span class='notice'>Will now be relocating \the [to_relocate].</span>", confidential = TRUE)
