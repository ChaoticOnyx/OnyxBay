...
/datum/techweb/proc/research_node(datum/techweb_node/node, force = FALSE, auto_adjust_cost = TRUE, get_that_dosh = TRUE)
	...
	update_node_status(node)
	if(get_that_dosh)
		var/datum/money_account/science_department_bank_account = department_accounts["Science"]
		log_message += ", gaining [TECHWEB_BOUNTY] to [science_department_bank_account.owner_name] for it."
		var/datum/transaction/T = new(science_department_bank_account.owner_name, log_message, TECHWEB_BOUNTY, "NTGalaxyNet Terminal #[rand(111,1111)]")
		science_department_bank_account.do_transaction(T)
