...
/obj/item/integrated_circuit/power/transmitter/large/do_work()
	if(..()) // If the above code succeeds, do this below.
		...
		if(prob(20))
			...
			spark_system.start()
			acting_object.visible_message(SPAN("warning", "\The [acting_object] makes some sparks!"))
		return TRUE
...
/obj/item/integrated_circuit/power/transmitter/wire_connector/do_work(ord)
	if(ord == 1)
