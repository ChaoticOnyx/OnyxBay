...
/obj/machinery/power/tracker/attackby(obj/item/weapon/W, mob/user)
	...
	if(isCrowbar(W))
		...
		user.visible_message(SPAN("notice", "[user] begins to take the glass off the solar tracker."))
		if(do_after(user, 50,src))
			...
			user.visible_message(SPAN("notice", "[user] takes the glass off the tracker."))
			qdel(src)
		return
	..()
