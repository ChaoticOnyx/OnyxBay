...
/datum/random_map/New(seed, tx, ty, tz, tlx, tly, do_not_apply, do_not_announce, never_be_priority = 0)
	...
	if(!do_not_announce) admin_notice(SPAN("danger", "Generating [name]."), R_DEBUG)
	CHECK_TICK
	...
	for(var/i = 0;i<max_attempts;i++)
		if(generate())
			if(!do_not_announce) admin_notice(SPAN("danger", "[capitalize(name)] generation completed in [round(0.1*(world.timeofday-start_time),0.1)] seconds."), R_DEBUG)
			return
	if(!do_not_announce) admin_notice(SPAN("danger", "[capitalize(name)] failed to generate ([round(0.1*(world.timeofday-start_time),0.1)] seconds): could not produce sane map."), R_DEBUG)
