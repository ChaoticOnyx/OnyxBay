...
/obj/machinery/shipsensors/examine(mob/user)
	. = ..()
	if(health <= 0)
		. = to_chat_or_concat(., user, "\The [src] is wrecked.")
	else if(health < max_health * 0.25)
		. = to_chat_or_concat(., user, "<span class='danger'>\The [src] looks like it's about to break!</span>")
	else if(health < max_health * 0.5)
		. = to_chat_or_concat(., user, "<span class='danger'>\The [src] looks seriously damaged!</span>")
