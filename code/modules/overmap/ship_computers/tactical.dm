...
/obj/machinery/computer/ship/tactical/tgui_interact(mob/user, datum/tgui/ui)
	if(isobserver(user))
		return

	if(!has_overmap())
		playsound(get_turf(src), pick('sound/signals/error1.ogg', 'sound/signals/error1.ogg', 'sound/signals/error1.ogg'), 100, 1)
	...
	if(!ui)
		...
		if(!linked.gunner && isliving(user))
			playsound(get_turf(src), SFX_TRR, 75, 1)
...
/obj/machinery/computer/ship/tactical/tgui_act(action, params, datum/tgui/ui)
	. = ..()
	if(.)
		return
...
/obj/machinery/computer/ship/tactical/tgui_data(mob/user)
	if(!linked)
		return

	var/list/data = list()
	data["flakrange"] = linked.get_flak_range(linked.last_target)
	data["integrity"] = linked.integrity
	...
	for(var/obj/structure/overmap/OM in linked.target_painted)
		if(OM.current_system != linked.current_system)
			continue
		data["painted_targets"] += list(list("name" = OM.name, "integrity" = OM.integrity, "max_integrity" = OM.max_integrity, \
...
/obj/machinery/computer/ship/tactical/internal/tgui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
...
/obj/machinery/computer/ship/tactical/internal/tgui_state(mob/user)
	return GLOB.tgui_always_state

/obj/machinery/computer/ship/tactical/internal/tgui_data(mob/user)
	if(!linked)
		return
