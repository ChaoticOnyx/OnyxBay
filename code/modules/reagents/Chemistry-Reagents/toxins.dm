...
/datum/reagent/toxin/mass_gainer/affect_touch(mob/living/carbon/human/M, alien, removed)
	...
	if(!alien)
		M.change_body_build(S.body_builds[HUMAN_FAT])
	else switch(alien)
		if(IS_SKRELL)
			M.change_body_build(S.body_builds[SKRELL_DEFAULT])
		if(IS_TAJARA)
			M.change_body_build(S.body_builds[TAJARA_FAT])
		else return
...
/datum/reagent/toxin/fat_burner/overdose(mob/living/carbon/human/M, alien, removed)
	var/datum/species/S = all_species[M.get_species()]
	if(!alien)
		if(M.body_build == S.body_builds[HUMAN_FAT])
			M.change_body_build(S.body_builds[HUMAN_DEFAULT])
		else if(M.body_build == S.body_builds[HUMAN_DEFAULT])
			M.gender == MALE ? M.change_body_build(S.body_builds[HUMAN_SLIM_MALE]) : M.change_body_build(S.body_builds[HUMAN_SLIM_FEMALE])
		else if(M.body_build == S.body_builds[HUMAN_SLIM_FEMALE])
			M.change_body_build(S.body_builds[HUMAN_SLIM_ALT_FEMALE])
		else return
	else switch(alien)
		if(IS_SKRELL)
			if(M.gender == FEMALE)
				if(M.body_build == S.body_builds[SKRELL_DEFAULT])
					M.change_body_build(S.body_builds[SKRELL_SLIM_FEMALE])
				if(M.body_build == S.body_builds[SKRELL_SLIM_FEMALE])
					M.change_body_build(S.body_builds[SKRELL_SLIM_ALT_FEMALE])
		if(IS_TAJARA)
			if(M.body_build == S.body_builds[TAJARA_FAT])
				M.change_body_build(S.body_builds[TAJARA_DEFAULT])
			if(M.body_build == S.body_builds[TAJARA_DEFAULT])
				if(M.gender == FEMALE)
					M.change_body_build(S.body_builds[TAJARA_SLIM_FEMALE])
			else return
		else return
