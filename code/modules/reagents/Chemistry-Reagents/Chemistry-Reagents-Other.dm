...
/datum/reagent/firefoam/touch_turf(turf/simulated/T)
	if(!istype(T))
		return
	...
	if(environment && environment.temperature > min_temperature)
		...
		if(prob(5))
			T.visible_message(SPAN("warning", "The foam sizzles as it lands on \the [T]!"))

/datum/reagent/firefoam/touch_mob(mob/living/L, amount)
	if(istype(L))
...
/datum/reagent/firefoam/affect_touch(mob/living/carbon/M, alien, removed)
	if(!istype(M, /mob/living/carbon/slime) && alien != IS_SLIME)
		return
	M.adjustToxLoss(10 * removed)	// The same as water, for balance raisins
	...
	if(M.chem_doses[type] == removed)
		M.visible_message(SPAN("warning", "[S]'s flesh sizzles where the foam touches it!"), SPAN("danger", "Your flesh burns in the foam!"))
