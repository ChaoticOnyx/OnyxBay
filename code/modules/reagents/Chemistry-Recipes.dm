...
/datum/chemical_reaction/proc/post_reaction(var/datum/reagents/holder)
	...
	if(mix_message && container && !ismob(container))
		...
		for(var/mob/M in seen)
			M.show_message("<span class='notice'>[icon2html(container, M, realsize=FALSE)] [mix_message]</span>", 1)
...
/datum/chemical_reaction/slime/on_reaction(var/datum/reagents/holder)
	...
	T.Uses--
	if(T.Uses <= 0)
		T.visible_message("[icon2html(T, viewers(T), realsize=FALSE)]<span class='notice'>\The [T]'s power is consumed in the reaction.</span>")
