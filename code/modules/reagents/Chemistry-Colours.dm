/datum/reagents/proc/get_color()
	...
	for(var/datum/reagent/R in reagent_list)
		...
		if(length(hex) != 9) // PANIC PANIC PANIC
			...
			hex = "#ffffffFF"
		colors[1] += hex2num(copytext_char(hex, 2, 4)) * R.volume * R.color_weight
		colors[2] += hex2num(copytext_char(hex, 4, 6)) * R.volume * R.color_weight
		colors[3] += hex2num(copytext_char(hex, 6, 8)) * R.volume * R.color_weight
		colors[4] += hex2num(copytext_char(hex, 8, 10)) * R.volume * R.color_weight
