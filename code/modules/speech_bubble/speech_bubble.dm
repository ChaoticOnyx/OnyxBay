/proc/fade_out_speech_bubble(image/I, list/remove_from)
	...
	addtimer(CALLBACK(GLOBAL_PROC, nameof(.proc/remove_image_from_clients), I, remove_from), 0.5 SECONDS)

/proc/animate_speech_bubble(image/I, list/show_to, duration)
	add_image_to_clients(I, show_to)
	addtimer(CALLBACK(GLOBAL_PROC, nameof(.proc/fade_out_speech_bubble), I, show_to), duration - 0.5 SECONDS)
...
/proc/show_bubble_to_clients(bubble_icon, bubble_icon_state, atom/source, list/show_to)
	...
	INVOKE_ASYNC(GLOBAL_PROC, nameof(.proc/animate_speech_bubble), speech_bubble, show_to, 3 SECONDS)
