...
/datum/tgui/proc/send_assets()
	var/flush_queue = window.send_asset(get_asset_datum(/datum/asset/simple/fontawesome))
	flush_queue |= window.send_asset(get_asset_datum(/datum/asset/simple/tgfont))
	if(flush_queue)
