...
/vs_control/proc/ChangeSetting(mob/user,ch)
	...
	to_world(SPAN("notice", "<b>[key_name(user)] changed the setting [display_description] to [newvar].</b>"))
	if(ch in plc.settings)
...
/vs_control/proc/SetDefault(mob/user)
	...
	to_world(SPAN("notice", "<b>[key_name(user)] changed the global plasma/ZAS settings to \"[def]\"</b>"))
