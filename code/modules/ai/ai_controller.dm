...
GLOBAL_LIST_EMPTY(ai_movements)
GLOBAL_LIST_EMPTY(ai_behaviors)
GLOBAL_LIST_EMPTY(ai_targeting_strategies)
...
/datum/ai_controller
	...
	var/datum/ai_movement/ai_movement = /datum/ai_movement/basic_avoidance
...
/datum/ai_controller/proc/init_behaviors()
	if(!length(GLOB.ai_subtrees.len))
		setup_subtrees()

	if(!length(GLOB.ai_movements.len))
		setup_ai_movements()

	if(!length(GLOB.ai_behaviors))
		setup_ai_behaviors()

	if(!length(GLOB.ai_targeting_strategies))
		setup_ai_targeting_strategies()
...
/datum/ai_controller/proc/change_ai_movement_type(datum/ai_movement/new_movement)
	ai_movement = GLOB.ai_movements[new_movement]
...
/datum/ai_controller/proc/PossessPawn(mob/new_pawn)
	...
	//SEND_SIGNAL(src, SIGNAL_AI_CONTROLLER_POSSESSED_PAWN)

	reset_ai_status()
	register_signal(pawn, SIGNAL_Z_CHANGED, nameof(.proc/on_changed_z_level))
	register_signal(pawn, SIGNAL_LOGGED_IN, nameof(.proc/on_sentience_gained))
	register_signal(pawn, SIGNAL_QDELETING, nameof(.proc/on_pawn_qdeleted))

	//register_signal(pawn, SIGNAL_MOVED, nameof(.proc/update_grid))
...
/datum/ai_controller/proc/UnpossessPawn(destroy)
	...
	unregister_signal(pawn, list(SIGNAL_Z_CHANGED, SIGNAL_LOGGED_IN, SIGNAL_LOGGED_OUT, SIGNAL_QDELETING))
	if(ai_movement.moving_controllers[src])
		ai_movement.stop_moving_towards(src)
...
/datum/ai_controller/think()
	...
	if(!LAZYLEN(current_behaviors) && idle_behavior)
		idle_behavior.perform_idle_behavior(src) //Do some stupid shit while we have nothing to do
		set_next_think(world.time + 0.5 SECONDS)
		return

	if(current_movement_target)
		if(!isatom(current_movement_target))
			util_crash_with("[pawn]'s current movement target is not an atom, rather a [current_movement_target.type]! Did you accidentally set it to a weakref?")
			CancelActions()
			set_next_think(world.time + 0.5 SECONDS)
			return
	...
	for(var/datum/ai_behavior/current_behavior as anything in current_behaviors)
		...
		if(current_behavior.behavior_flags & AI_BEHAVIOR_REQUIRE_MOVEMENT) //Might need to move closer
			...
			var/can_reach = !(current_behavior.behavior_flags & AI_BEHAVIOR_REQUIRE_REACH) || moving_pawn.Adjacent(moving_pawn, current_movement_target)
...
/datum/ai_controller/proc/SelectBehaviors()
	...
	if(LAZYLEN(planning_subtrees))
		for(var/datum/ai_planning_subtree/subtree as anything in planning_subtrees)
			if(subtree.SelectBehaviors(src) == SUBTREE_RETURN_FINISH_PLANNING)
				break

	SEND_SIGNAL(src, SIGNAL_AI_CONTROLLER_PICKED_BEHAVIORS, current_behaviors, planned_behaviors)
...
/datum/ai_controller/proc/on_sentience_gained()
	SIGNAL_HANDLER
	unregister_signal(pawn, SIGNAL_LOGGED_IN)
	if(!continue_processing_when_client)
		set_ai_status(AI_STATUS_OFF) //Can't do anything while player is connected
	register_signal(pawn, SIGNAL_LOGGED_OUT, nameof(.proc/on_sentience_lost))

/datum/ai_controller/proc/on_sentience_lost()
	SIGNAL_HANDLER
	unregister_signal(pawn, SIGNAL_LOGGED_OUT)
...
#define TRACK_AI_DATUM_TARGET(tracked_datum, key) do { \
	if(isweakref(tracked_datum)) { \
		var/weakref/_bad_weakref = tracked_datum; \
		util_crash_with("Weakref (Actual datum: [_bad_weakref.resolve()]) found in ai datum blackboard! \
			This is an outdated method of ai reference handling, please remove it."); \
	}; \
	else if(istype(tracked_datum, /datum)) { \
		var/datum/_tracked_datum = tracked_datum; \
		if(!HAS_TGS_TRAIT_FROM(_tracked_datum, TRAIT_AI_TRACKING, "[any2ref(src)]_[key]")) { \
			register_signal(_tracked_datum, SIGNAL_QDELETING, nameof(.proc/sig_remove_from_blackboard), override = TRUE); \
			ADD_TGS_TRAIT(_tracked_datum, TRAIT_AI_TRACKING, "[any2ref(src)]_[key]"); \
		}; \
	}; \
} while(FALSE)
...
#define CLEAR_AI_DATUM_TARGET(tracked_datum, key) do { \
	if(istype(tracked_datum, /datum)) { \
		var/datum/_tracked_datum = tracked_datum; \
		REMOVE_TGS_TRAIT(_tracked_datum, TRAIT_AI_TRACKING, "[any2ref(src)]_[key]"); \
		if(!HAS_TGS_TRAIT(_tracked_datum, TRAIT_AI_TRACKING)) { \
			unregister_signal(_tracked_datum, SIGNAL_QDELETING); \
		}; \
	}; \
} while(FALSE)
...
/datum/ai_controller/proc/post_blackboard_key_set(key)
	if (isnull(pawn))
		return
	SEND_SIGNAL(pawn, SIGNAL_AI_BLACKBOARD_KEY_SET(key))
...
/datum/ai_controller/proc/clear_blackboard_key(key)
	...
	if(isnull(pawn))
		return
	SEND_SIGNAL(pawn, SIGNAL_AI_BLACKBOARD_KEY_CLEARED(key))
...
/datum/ai_controller/proc/remove_thing_from_blackboard_key(key, thing)
	...
	if(thing == associated_value)
		util_crash_with("remove_thing_from_blackboard_key was called un-necessarily in a situation where clear_blackboard_key would suffice. ")
		clear_blackboard_key(key)
		return
	...
	while(index <= length(remove_queue))
		...
		for(var/inner_value in next_to_clear)
			...
			// We found the value that's been deleted, it was an assoc value. Clear it out entirely
				...
				next_to_clear -= inner_value
				SEND_SIGNAL(pawn, SIGNAL_AI_BLACKBOARD_KEY_CLEARED(inner_value))
