...
/datum/category_item/player_setup_item/loadout/proc/toggle_gear(datum/gear/TG, mob/user)
	...
	ASSERT(user.client.donator_info.patron_type_tier_discount_applied(TG.price) || (!TG.price || user.client.donator_info.has_item(TG.type)))
...
/datum/gear/proc/is_allowed_to_equip(mob/user)
	...
	if(price && (!user.client.donator_info.patron_type_tier_discount_applied(price) && !user.client.donator_info.has_item(type)))
		return FALSE
