...
/datum/category_item/player_setup_item/general/flavor/OnTopic(href,list/href_list, mob/user)
	if(href_list["flavor_text"])
		switch(href_list["flavor_text"])
			if("open")
			if("general")
				var/msg = sanitize(input_cp1251(usr,"Give a general description of your character. This will be shown regardless of clothing.","Flavor Text",rhtml_decode(pref.flavor_texts[href_list["flavor_text"]])), extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
			else
				var/msg = sanitize(input_cp1251(usr,"Set the flavor text for your [href_list["flavor_text"]].","Flavor Text",rhtml_decode(pref.flavor_texts[href_list["flavor_text"]])), extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
		SetFlavorText(user)
		...
		switch(href_list["flavour_text_robot"])
			if("open")
			if("Default")
				var/msg = sanitize(input_cp1251(usr,"Set the default flavour text for your robot. It will be used for any module without individual setting.","Flavour Text",rhtml_decode(pref.flavour_texts_robot["Default"])), extra = 0)
				if(CanUseTopic(user))
