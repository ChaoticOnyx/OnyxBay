...
/datum/category_item/player_setup_item/general/background/OnTopic(href,list/href_list, mob/user)
	if(href_list["nt_relation"])
		...
		var/new_general = sanitize(tgui_input_pencode_editor(user,"Enter employment information here.","Character Preference", html_decode(pref.gen_record)))
		if(!isnull(new_general) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.gen_record = new_general
		return TOPIC_REFRESH

	else if(href_list["set_security_records"])
		var/sec_medical = sanitize(tgui_input_pencode_editor(user,"Enter security information here.","Character Preference", html_decode(pref.sec_record)))
		if(!isnull(sec_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.sec_record = sec_medical
		return TOPIC_REFRESH

	else if(href_list["exploitable_record"])
		var/exploitmsg = sanitize(tgui_input_pencode_editor(user,"Set exploitable information about you here.","Exploitable Information", html_decode(pref.exploit_record)))
