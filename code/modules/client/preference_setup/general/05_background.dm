...
/datum/category_item/player_setup_item/general/background/OnTopic(href,list/href_list, mob/user)
	if(href_list["nt_relation"])
		...
		var/new_medical = sanitize(tgui_input_pencode_editor(user,"Enter medical information here.","Character Preference", html_decode(pref.med_record)), MAX_PAPER_MESSAGE_LEN, extra = 0)
		if(!isnull(new_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.med_record = new_medical
		return TOPIC_REFRESH

	else if(href_list["set_general_records"])
		var/new_general = sanitize(tgui_input_pencode_editor(user,"Enter employment information here.","Character Preference", html_decode(pref.gen_record)), MAX_PAPER_MESSAGE_LEN, extra = 0)
		if(!isnull(new_general) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.gen_record = new_general
		return TOPIC_REFRESH

	else if(href_list["set_security_records"])
		var/sec_medical = sanitize(tgui_input_pencode_editor(user,"Enter security information here.","Character Preference", html_decode(pref.sec_record)), MAX_PAPER_MESSAGE_LEN, extra = 0)
		if(!isnull(sec_medical) && !jobban_isbanned(user, "Records") && CanUseTopic(user))
			pref.sec_record = sec_medical
		return TOPIC_REFRESH

	else if(href_list["exploitable_record"])
		var/exploitmsg = sanitize(tgui_input_pencode_editor(user,"Set exploitable information about you here.","Exploitable Information", html_decode(pref.exploit_record)), MAX_PAPER_MESSAGE_LEN, extra = 0)
		...
		var/memes = sanitize(tgui_input_pencode_editor(user,"Enter memorized information here.","Character Preference", html_decode(pref.memory)), MAX_PAPER_MESSAGE_LEN, extra = 0)
