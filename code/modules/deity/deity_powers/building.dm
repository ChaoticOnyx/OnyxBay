...
/mob/living/deity/proc/set_selected_power(datum/deity_power/power_to_select, force = FALSE)
	if(istype(selected_power) && !selected_power.can_switch && !force)
		return

	selected_power = power_to_select
	var/datum/hud/deity/D = hud_used
	D.update_selected(selected_power)
	if(power_to_select.manifest_on_select)
		power_to_select.manifest(src, src)
...
/obj/structure/deity
	...
	var/activation_cost_resource = null
	var/activation_cost_amount

	density = TRUE
	anchored = TRUE

/obj/structure/deity/Initialize(mapload, owner, health)
	. = ..()
	if(owner)
		linked_deity = owner
...
/obj/structure/deity/proc/can_activate(mob/living/deity/D, warning = TRUE)
	if(last_click + click_cooldown < world.time && D == linked_deity)
		if(!isnull(activation_cost_resource) && !linked_deity.form.use_resource(activation_cost_resource, activation_cost_amount))
			if(warning)
				show_splash_text(linked_deity, "Not enough points!", SPAN_WARNING("You need more [activation_cost_resource] to activate \the [src]!"))
			return FALSE
