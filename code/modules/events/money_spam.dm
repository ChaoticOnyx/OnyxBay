	...
	if(useMS)
		if(prob(5))
			...
			for (var/mob/O in hearers(3, P.loc))
				if(!P.message_silent) O.show_message(text("[icon2html(P, world, realsize=FALSE)] *[P.ttone]*"))
			...
			if(L)
				to_chat(L, "[icon2html(P, world, realsize=FALSE)] <b>Message from [sender] (Unknown / spam?), </b>\"[message]\" (Unable to Reply)")
