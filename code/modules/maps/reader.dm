 ...
 *		e.g aa = /turf/wall{icon_state = "rock"}
 ...
 * Variable model is one full map line (e.g /turf/wall{icon_state = "rock"}, /area/mine/explored)
 *
 * WORKING :
 *
...
/dmm_suite/proc/parse_grid(model as text, model_key as text, xcrd as num,ycrd as num,zcrd as num, no_changeturf as num, clear_contents as num, turf_blacklist)
	...
	*/
	...
	var/list/members //will contain all members (paths) in model (in our example : /turf/wall and /area/mine/explored)
	...
	if(cached)
		...
		do
			//finding next member (e.g /turf/wall{icon_state = "rock"} or /area/mine/explored)
