...
/obj/machinery/ship_weapon //CREDIT TO CM FOR THE SPRITES!
	...
	var/load_sound = 'sound/effects/ship/mac_load.ogg'
	var/mag_load_sound = 'sound/effects/weapons/gun/interaction/autoguninsert.ogg'
	var/load_delay = 20

	var/unload_sound = 'sound/effects/ship/freespace2/crane_short.ogg'
	var/mag_unload_sound = 'sound/effects/weapons/gun/interaction/autoguninsert.ogg'
	var/unload_delay = 10

	var/feeding_sound = 'sound/effects/ship/freespace2/crane_short.ogg'
	var/fed_sound = 'sound/effects/ship/reload.ogg'
	var/feed_delay = 20

	var/chamber_sound = 'sound/effects/weapons/gun/railgun/ready.ogg'
	var/chamber_delay_rapid = 2
	var/chamber_delay = 10

	var/firing_sound = 'sound/effects/ship/tri_mount_fire.ogg'
	...
	var/obj/item/ammo_magazine/magazine //Magazine if we have one
...
/obj/machinery/ship_weapon/Initialize(mapload)
	. = ..()
	add_think_ctx("get_ship", CALLBACK(src, nameof(.proc/get_ship)), world.time + 15 SECONDS)
	return INITIALIZE_HINT_LATELOAD

/obj/machinery/ship_weapon/LateInitialize()
	if(maintainable)
		maint_req = rand(20,25)
		create_reagents(50)
 ...
 */
/obj/machinery/ship_weapon/Destroy(force)
...
/obj/machinery/ship_weapon/examine(mob/user, infix)
	. = ..()
	if(malfunction)
...
/obj/machinery/ship_weapon/attackby(obj/item/I, mob/user)
	...
	if(isScrewdriver(I))
		if(state == STATE_FIRING && maint_state == MSTATE_CLOSED)
			to_chat(user, SPAN_WARNING("You cannot open the maintence panel while [src] is firing!"))
			return TRUE

		else if(state < STATE_FIRING && maint_state == MSTATE_CLOSED)
			to_chat(user, SPAN_NOTICE("You begin unfastening the maintenance panel on [src]..."))
			if(!do_after(user, 4 SECONDS, src, TRUE, TRUE) || QDELETED(src))
				return

			to_chat(user, SPAN_NOTICE(" You unfasten the maintenance panel on [src]."))
			...
			to_chat(user, SPAN_NOTICE("You begin fastening the maintenance panel on [src]..."))
			if(!do_after(user, 4 SECONDS, src, TRUE, TRUE) || QDELETED(src))
				return

			to_chat(user, SPAN_NOTICE(" You fasten the maintenance panel on [src]."))
	...
	if(isWrench(I))
		if(state >= STATE_CHAMBERED && maint_state == MSTATE_UNSCREWED)//we can put buttons and stuff inside the maintenance panel
			to_chat(user, SPAN_WARNING("You cannot unbolt the inner casing while [src] has a round chambered!"))
			return TRUE

		else if(maint_state == MSTATE_UNSCREWED)
			to_chat(user, SPAN_NOTICE("You begin unfastening the inner casing bolts on [src]..."))
			if(!do_after(user, 4 SECONDS, src, TRUE, TRUE) || QDELETED(src))
				return

			to_chat(user, SPAN_NOTICE("You unfasten the inner case bolts on [src] and remove the inner casing."))
			maint_state = MSTATE_UNBOLTED
			update_icon()
			return TRUE

		else if(maint_state == MSTATE_UNBOLTED)
			to_chat(user, SPAN_NOTICE("You begin adding the inner casing and fastening the inner casing bolts on [src]..."))
			if(!do_after(user, 4 SECONDS, src, TRUE, TRUE) || QDELETED(src))
				return

			to_chat(user, SPAN_NOTICE("You fasten the inner case bolts on [src]."))
			maint_state = MSTATE_UNSCREWED
			update_icon()
			return TRUE
	...
	if(isCrowbar(I))
		if(maint_state != MSTATE_UNBOLTED)
			to_chat(user, SPAN_WARNING("The inner casing bolts are fastened on [src]!"))
			return FALSE
	...
	if(isMultitool(I))
		...
		P.buffer_object = src
		to_chat(user, SPAN_NOTICE("-% Successfully stored [any2ref(P.buffer_object)] [P.buffer_name] in buffer %-"))
		return TRUE
...
/obj/machinery/ship_weapon/proc/load(obj/A, mob/user)
	...
	if(length(ammo) < max_ammo) //Room for one more?
		if(!loading) //Not already loading a round?
			if(user)
				to_chat(user, SPAN_NOTICE("You start to load [A] into [src]..."))
			loading = TRUE
			if(!user || do_after(user, load_delay, target = src))
				if(!isturf(A.loc) && !ismob(A.loc)) //Fix double-loading torpedos
					if(user)
						loading = FALSE
						to_chat(user, SPAN_WARNING("The ammunition has to be next to the weapon!"))
					return FALSE
			...
			to_chat(user, SPAN_NOTICE("You're already loading a round into [src]!."))
	else if(user)
		to_chat(user, SPAN_WARNING("[src] is already fully loaded!"))

	return FALSE

/// Returns max ammo for tactical consoles.
/obj/machinery/ship_weapon/proc/get_max_ammo()
	return max_ammo

/// Returns current ammo for tactical consoles.
/obj/machinery/ship_weapon/proc/get_ammo()
	return length(ammo)

/// STATE_NOTLOADED to STATE_LOADED. Returns TRUE if loaded successfully, FALSE if not.
/obj/machinery/ship_weapon/proc/load_magazine(obj/A, mob/user)
	...
	if(magazine_type && istype(A, magazine_type))
		...
		to_chat(user, SPAN_WARNING("You can't load [A] into [src]!"))

	return FALSE

/// Ejects round(s) from the weapon.
...
/obj/machinery/ship_weapon/proc/unload_magazine(mob/user)
	if((state >= STATE_LOADED) && magazine)
		to_chat(user, SPAN_NOTICE("You start to unload [magazine] from [src]."))
		if(state >= STATE_FED)
...
/obj/machinery/ship_weapon/proc/local_fire()
	...
	/*
	if(bang)
		for(var/mob/living/carbon/M in hearers(bang_range, src)) //Burst unprotected eardrums
			if(M.stat != DEAD && M.get_ear_protection() < 1) //checks for protection - why was this not here before???
				M.soundbang_act(1, 200, 10, 15)
	*/

/// Plays firing animations and sounds, applies them to the overmap avatar
