...
/obj/item/gun/energy/temperature/_examine_text(mob/user)
	. = ..()
	. += "\nThe temperature sensor shows: [round(convert_k2c(temperature))]&deg;C"
...
/obj/item/gun/energy/temperature/attack_self(mob/living/user)
	...
	if(temperature > (-50 CELSIUS))
		temp_text = "<FONT color=black>[temperature] ([round(convert_k2c(temperature))]&deg;C) ([round(temperature*1.8-459.67)]&deg;F)</FONT>"
	else
		temp_text = "<FONT color=blue>[temperature] ([round(convert_k2c(temperature))]&deg;C) ([round(temperature*1.8-459.67)]&deg;F)</FONT>"
