...
/obj/item/weapon/gun/energy/plasmacutter/attackby(obj/item/stack/material/phoron/W, mob/user)
	...
	if(current_power < max_shots && danger_attack == TRUE)
		...
		W.use(1)
		to_chat(user, SPAN_NOTICE("You insert [W.material.use_name] [W.material.sheet_singular_name] into [src]."))
	else
		to_chat(user, SPAN_WARNING("You can't insert [W.material.use_name] [W.material.sheet_singular_name] into [src], it's full."))
