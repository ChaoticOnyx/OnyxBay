...
/obj/machinery/suspension_gen/proc/activate()
	...
	for(var/mob/living/M in T)
		M.weakened += 5
		M.visible_message("<span class='notice'>[icon2html(M, viewers(M), realsize=FALSE)] [M] begins to float in the air!</span>","You feel tingly and light, but it is difficult to move.")

	suspension_field = new(T)
	src.visible_message("<span class='notice'>[icon2html(src, viewers(src), realsize=FALSE)] [src] activates with a low hum.</span>")
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.overlays += "shield2"
		src.visible_message("<span class='notice'>[icon2html(suspension_field, viewers(src), realsize=FALSE)] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"].</span>")
	else
...
/obj/machinery/suspension_gen/proc/deactivate()
	...
	src.visible_message("<span class='notice'>[icon2html(src, viewers(src), realsize=FALSE)] [src] deactivates with a gentle shudder.</span>")
