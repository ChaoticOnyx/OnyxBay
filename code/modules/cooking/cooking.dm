...
/datum/cooking/recipe/proc/set_step_desc(var/new_description)
	last_created_step.desc = new_description

/datum/cooking/recipe/proc/set_step_max_quality(var/quality)
	last_created_step.flags |= CWJ_BASE_QUALITY_ENABLED
	last_created_step.max_quality_award = quality

/datum/cooking/recipe/proc/set_step_base_quality(var/quality)
	last_created_step.flags |= CWJ_MAX_QUALITY_ENABLED
	last_created_step.base_quality_award = quality

/datum/cooking/recipe/proc/set_step_custom_result_desc(var/new_description)
	last_created_step.custom_result_desc = new_description


/datum/cooking/recipe/proc/set_exact_type_required(var/boolean)
...
/datum/cooking/recipe/proc/set_reagent_skip(var/boolean)
...
/datum/cooking/recipe/proc/set_exclude_reagents(var/list/exclude_list)
	if((last_created_step.class == CWJ_ADD_ITEM) || (last_created_step.class == CWJ_ADD_PRODUCE))
...
/datum/cooking/recipe/proc/set_inherited_quality_modifier(var/modifier)
...
/datum/cooking/recipe/proc/set_remain_percent_modifier(var/modifier)
...
/datum/cooking/recipe/proc/add_step(var/datum/cooking/recipe_step/step, var/optional)
...
/datum/cooking/recipe/proc/calculate_reagent_quality(var/datum/cooking/recipe_pointer/pointer)
	if(!GLOB.cwj_step_dictionary_ordered["[CWJ_ADD_REAGENT]"])
		return 0
	var/datum/cooking/recipe_tracker/parent = pointer.parent_ref.resolve()
	var/obj/item/container = parent.holder_ref.resolve()
...
//-----------------------------------------------------------------------------------
/datum/cooking/proc/get_class_string(var/code)
	switch(code)
		if(CWJ_ADD_ITEM)
			return "Add Item"
		if(CWJ_USE_ITEM)
