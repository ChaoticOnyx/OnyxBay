...
/datum/cooking/recipe/proc/build_steps()
	...
	if(exclusive_option_mode)
		CRASH("/datum/cooking/recipe/New: Exclusive option active at end of recipe creation process. Recipe name=[name].")

	if(option_chain_mode)
		CRASH("/datum/cooking/recipe/New: Option Chain active at end of recipe creation process. Recipe name=[name].")

	if(last_created_step.flags & CWJ_IS_OPTIONAL)
		CRASH("/datum/cooking/recipe/New: Last option in builder is optional. It must be a required step! Recipe name=[name].")
...
/datum/cooking/recipe/proc/create_step_add_reagent(reagent_id, amount, optional)
...
/datum/cooking/recipe/proc/create_step_add_item(item_type, optional)
	var/datum/cooking/recipe_step/add_item/step = new (item_type, src)
	return src.add_step(step, optional)
//-----------------------------------------------------------------------------------
//Use item step shortcut commands
/datum/cooking/recipe/proc/create_step_use_item(item_type, optional)
	var/datum/cooking/recipe_step/use_item/step = new (item_type, src)
	return src.add_step(step, optional)

//-----------------------------------------------------------------------------------
//Add produce step shortcut commands
/datum/cooking/recipe/proc/create_step_add_produce(produce, optional)
...
/datum/cooking/recipe/proc/set_reagent_skip(boolean)
...
/datum/cooking/recipe/proc/set_exclude_reagents(list/exclude_list)
	if((last_created_step.class == CWJ_ADD_ITEM) || (last_created_step.class == CWJ_ADD_PRODUCE))
...
/datum/cooking/recipe/proc/set_inherited_quality_modifier(modifier)
...
/datum/cooking/recipe/proc/set_remain_percent_modifier(modifier)
...
/datum/cooking/recipe/proc/begin_exclusive_options()
	if(exclusive_option_mode)
		#ifdef CWJ_DEBUG
		log_debug("/datum/cooking/recipe/proc/begin_exclusive_options: Exclusive option already active.")
		log_debug("Recipe name=[name].")
		#endif
		return
	else if(!first_step)
		CRASH("/datum/cooking/recipe/proc/begin_exclusive_options: Exclusive list cannot be active before the first required step is defined. Recipe name=[src.type].")
	exclusive_option_mode = TRUE
	active_exclusive_option_list = list()

/datum/cooking/recipe/proc/end_exclusive_options()
	if(!exclusive_option_mode)
		#ifdef CWJ_DEBUG
		log_debug("/datum/cooking/recipe/proc/end_exclusive_options: Exclusive option already inactive.")
		log_debug("Recipe name=[name].")
		#endif
		return
	else if(active_exclusive_option_list.len == 0)
		CRASH("/datum/cooking/recipe/proc/end_exclusive_options: Exclusive option list ended with no values added. Recipe name=[src.type].")
	else if(option_chain_mode)
		CRASH("/datum/cooking/recipe/proc/end_exclusive_options: Exclusive option cannot end while option chain is active. Recipe name=[src.type].")
	...
	for (var/datum/cooking/recipe_step/exclusive_option in active_exclusive_option_list)
	...
	for (var/datum/cooking/recipe_step/exclusive_option in active_exclusive_option_list)
		for (var/datum/cooking/recipe_step/excluder in active_exclusive_option_list["[exclusive_option]"])
...
/datum/cooking/recipe/proc/begin_option_chain()
	if(option_chain_mode)
		#ifdef CWJ_DEBUG
		log_debug("/datum/cooking/recipe/proc/begin_option_chain: Option Chain already active.")
		log_debug("Recipe name=[name].")
		#endif
		return
	if(!first_step)
		CRASH("/datum/cooking/recipe/proc/begin_option_chain: Option Chain cannot be active before first required step is defined. Recipe name=[name].")
	option_chain_mode =1

/datum/cooking/recipe/proc/end_option_chain()
	if(!option_chain_mode)
		#ifdef CWJ_DEBUG
		log_debug("/datum/cooking/recipe/proc/end_option_chain: Option Chain already inactive.")
		log_debug("Recipe name=[name].")
		#endif
		return
...
/datum/cooking/recipe/proc/add_step(datum/cooking/recipe_step/step, optional)
	...
	if(!optional && exclusive_option_mode)
		CRASH("/datum/cooking/recipe/proc/add_step: Required step added while exclusive option mode is on. Recipe name=[name].")

	if(!optional && option_chain_mode)
		CRASH("/datum/cooking/recipe/proc/add_step: Required step added while option chain mode is on. Recipe name=[name].")
	...
	if(exclusive_option_mode)
		...
		for (var/datum/cooking/recipe_step/ex_step in active_exclusive_option_list)
...
/datum/cooking/recipe/proc/create_product(datum/cooking/recipe_pointer/pointer)
	var/datum/cooking/recipe_tracker/parent = pointer.parent_ref.resolve()
...
/datum/cooking/recipe/proc/calculate_reagent_quality(datum/cooking/recipe_pointer/pointer)
	if(!GLOB.cwj_step_dictionary_ordered["[CWJ_ADD_REAGENT]"])
		return 0
	var/datum/cooking/recipe_tracker/parent = pointer.parent_ref.resolve()
	var/obj/item/container = parent.holder_ref.resolve()
...
//-----------------------------------------------------------------------------------
/datum/cooking/proc/get_class_string(code)
	switch(code)
		if(CWJ_ADD_ITEM)
			return "Add Item"
		if(CWJ_USE_ITEM)
