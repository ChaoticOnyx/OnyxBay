...
/datum/computer_file/program/proc/event_networkfailure(background)
	kill_program(1)
	if(background)
		computer.visible_message(SPAN("warning", "\The [computer]'s screen displays an error: \"Network connectivity lost - process [filename].[filetype] (PID [rand(100,999)]) terminated.\""), range = 1)
	else
		computer.visible_message(SPAN("warning", "\The [computer]'s screen briefly freezes and then shows: \"FATAL NETWORK ERROR - NTNet connection lost. Please try again later. If problem persists, please contact your system administrator.\""), range = 1)
