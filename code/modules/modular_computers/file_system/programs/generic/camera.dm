...
/datum/computer_file/program/camera_monitor
	...
	nanomodule_path = /datum/onyxui_module/camera_monitor
...
/datum/onyxui_module/camera_monitor
	name = "Camera Monitoring program"
	var/obj/machinery/camera/current_camera = null
	var/current_network = null
	var/last_sound = 0

/datum/onyxui_module/camera_monitor/ui_interact(mob/user, ui_key = "main", datum/onyxui/ui = null, force_open = 1, state = GLOB.default_state)
...
/datum/onyxui_module/camera_monitor/proc/modify_networks_list(list/networks)
	return networks

/datum/onyxui_module/camera_monitor/proc/can_access_network(mob/user, network_access)
...
/datum/onyxui_module/camera_monitor/Topic(href, href_list)
	if(..())
		return 1
	...
	if(href_list["switch_camera"])
		...
		if(can_access_network(usr, get_camera_access(href_list["switch_network"])))
			...
			to_chat(usr, "\The [onyxui_host()] shows an \"Network Access Denied\" error message.")
		return 1
...
/datum/onyxui_module/camera_monitor/proc/switch_to_camera(mob/user, obj/machinery/camera/C)
	...
	set_current(C)
	user.machine = onyxui_host()
	user.reset_view(C)
	return 1

/datum/onyxui_module/camera_monitor/proc/set_current(obj/machinery/camera/C)
	if(current_camera == C)
		return
...
/datum/onyxui_module/camera_monitor/proc/reset_current()
	if(current_camera)
		var/mob/living/L = current_camera.loc
		if(istype(L))
...
/datum/onyxui_module/camera_monitor/check_eye(mob/user as mob)
	if(!current_camera)
		return 0
...
/datum/computer_file/program/camera_monitor/ert
	...
	size = 14
	nanomodule_path = /datum/onyxui_module/camera_monitor/ert
	available_on_ntnet = 0

/datum/onyxui_module/camera_monitor/ert
...
/datum/onyxui_module/camera_monitor/ert/modify_networks_list(list/networks)
	..()
	networks.Add(list(list("tag" = NETWORK_ERT, "has_access" = 1)))
	networks.Add(list(list("tag" = NETWORK_CRESCENT, "has_access" = 1)))
	return networks

/datum/onyxui_module/camera_monitor/apply_visual(mob/M)
	if(current_camera)
		current_camera.apply_visual(M)
	else
		remove_visual(M)

/datum/onyxui_module/camera_monitor/remove_visual(mob/M)
	if(current_camera)
