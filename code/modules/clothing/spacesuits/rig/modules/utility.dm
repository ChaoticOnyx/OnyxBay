...
/obj/item/rig_module/chem_dispenser/accepts_item(obj/item/input_item, mob/living/user)
	...
	if(total_transferred)
		to_chat(user, "<span class='info'>You transfer [total_transferred] ml into the suit reservoir.</span>")
	else
...
/obj/item/rig_module/chem_dispenser/engage(atom/target)
	...
	if(target_mob != H)
		to_chat(H, "<span class='danger'>You inject [target_mob] with [chems_to_use] ml of [charge.display_name].</span>")
	to_chat(target_mob, "<span class='danger'>You feel a rushing in your veins as [chems_to_use] ml of [charge.display_name] [chems_to_use == 1 ? "is" : "are"] injected.</span>")
