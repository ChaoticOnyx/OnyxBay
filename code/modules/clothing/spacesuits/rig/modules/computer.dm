...
/obj/item/rig_module/ai_container/accepts_item(obj/item/input_device, mob/living/user)
	...
	if(input_device.type in list(/obj/item/device/paicard, /obj/item/organ/internal/mastermind/mmi, /obj/item/organ/internal/mastermind/posibrain))
		if(integrated_ai)
			integrated_ai.attackby(input_device,user)
			// If the transfer was successful, we can clear out our vars.
			if(integrated_ai.loc != src)
...
/obj/item/rig_module/datajack/accepts_item(obj/item/input_device, mob/living/user)
	...
	else if(istype(input_device,/obj/machinery/computer/rdconsole) || istype(input_device,/obj/machinery/r_n_d/server))
