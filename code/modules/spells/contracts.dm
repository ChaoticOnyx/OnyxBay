...
/obj/item/weapon/contract/attack_self(mob/user as mob)
	if(contract_master == null)
		to_chat(user, SPAN("notice", "You bind the contract to your soul, making you the recipient of whatever poor fool's soul that decides to contract with you."))
...
/obj/item/weapon/contract/proc/contract_effect(mob/user as mob)
	to_chat(user, SPAN("warning", "You've signed your soul over to \the [contract_master] and with that your unbreakable vow of servitude begins."))
	return 1
...
/obj/item/weapon/contract/apprentice/contract_effect(mob/user as mob)
	if(user.mind.special_role == "apprentice")
		to_chat(user, SPAN("warning", "You are already a wizarding apprentice!"))
		return 0
	if(GLOB.wizards.add_antagonist_mind(user.mind,1,"apprentice","<b>You are an apprentice! Your job is to learn the wizarding arts!</b>"))
		to_chat(user, SPAN("notice", "With the signing of this paper you agree to become \the [contract_master]'s apprentice in the art of wizardry."))
		return 1
	return 0
...
/obj/item/weapon/contract/wizard/xray/contract_effect(mob/user as mob)
	...
	if (!(MUTATION_XRAY in user.mutations))
		...
		to_chat(user, SPAN("notice", "The walls suddenly disappear."))
		return 1
	return 0
...
/obj/item/weapon/contract/wizard/telepathy/contract_effect(mob/user as mob)
	...
	if(!(mRemotetalk in user.mutations))
		...
		to_chat(user, SPAN("notice", "You expand your mind outwards."))
		return 1
	return 0
...
/obj/item/weapon/contract/wizard/tk/contract_effect(mob/user as mob)
	...
	if(!(MUTATION_TK in user.mutations))
		...
		to_chat(user, SPAN("notice", "You feel your mind expanding!"))
		return 1
	return 0
