...
/datum/spell/targeted/equip_item
	...
	var/delete_old = FALSE //if the item previously in the slot is deleted - otherwise, it's dropped
	...
	for(var/mob/living/L in targets)
		for(var/slot_id in equipped_summons)
			...
			if(cmptext(slot_id,"active hand"))
				slot_id = (user.active_hand == ACTIVE_HAND_LEFT ? slot_l_hand : slot_r_hand)
			else if(cmptext(slot_id, "off hand"))
				slot_id = (user.active_hand == ACTIVE_HAND_LEFT ? slot_r_hand : slot_l_hand)
			else
