...
/spell/hand/charges/blood_shard/cast_hand(atom/A,mob/user)
	...
	B.firer = user
	B.launch(A, BP_CHEST)
	user.visible_message(SPAN("danger", "\The [user] shoots out a deep red shard from their hand!"))
	return ..()
...
/obj/item/projectile/blood_shard/on_hit(atom/movable/target, blocked = 0)
	if(..())
		if(istype(target, /mob/living/carbon/human))
			...
			H.visible_message(SPAN("danger", "Tiny red shards burst from \the [H]'s skin!"))
