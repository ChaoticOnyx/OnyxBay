...
/mob/new_player/proc/handle_player_polling()
	...
	var/DBQuery/select_query = sql_query({"
		SELECT
			id,
			question
		FROM
...
/mob/new_player/proc/poll_player(pollid = -1)
	...
	switch(polltype)
		...
		if("OPTION")
			var/DBQuery/voted_query = sql_query({"
				SELECT
					optionid
				FROM
					erro_poll_vote
				WHERE
					...
					AND
					ckey = $ckey
				"}, dbcon, list(pollid = pollid, ckey = usr.ckey))
			...
			var/DBQuery/options_query = sql_query({"
				SELECT
					id,
					text
				FROM
		...
		if("NUMVAL")
			...
			if(!voted)	//Only make this a form if we have not voted yet
				...
				var/DBQuery/option_query = sql_query({"
					SELECT
						id,
						text,
						minval,
...
/mob/new_player/proc/vote_on_numval_poll(pollid = -1, optionid = -1, rating = null)
	...
	var/DBQuery/voted_query = sql_query({"
		...
		WHERE
			...
			AND
			ckey = $ckey
		"}, dbcon, list(optionid = optionid, ckey = usr.ckey))
