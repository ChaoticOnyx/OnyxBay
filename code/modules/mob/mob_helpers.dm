...
proc/slur(phrase)
	...
	while(counter>=1)
		...
		if(rand(1,3)==3)
			...
			if(lowertext(newletter)=="ё")	newletter="о"
			if(lowertext(newletter)=="с")	newletter="ч"
			if(lowertext(newletter)=="а")	newletter="aх"
			if(lowertext(newletter)=="к")	newletter="г"
		switch(rand(1,15))
...
/proc/stutter(n)
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		...
		if (prob(80) && (lowertext(n_letter) in list("б","в","г","д","ж","з","к","л","м","н","п","р","с","т","ф","х","ц","ч","ш","щ","b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","y","z")))
			if (prob(10))
