...
/proc/stutter(n)
	...
	var/p = null
	var/shutterable_letters = list("б","в","г","д","ж","з","й","к","л","м","н","п","р","с","т","ф","х","ц","ч","ш","щ","b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","y","z");
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		...
		var/shutterable = 0;
		var/letter = null;
		for (letter in shutterable_letters)
			if (letter == n_letter)
				shutterable = 1;

		if (prob(80) && (shutterable == 1))
			if (prob(10))
