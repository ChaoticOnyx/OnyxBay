...
/datum/wound/proc/heal_damage(amount)
	...
	if(parent_organ)
		if(damage_type == BURN && !(parent_organ.burn_ratio < 1 || (parent_organ.organ_flags & ORGAN_FLAG_HEALS_OVERKILL)))
			return amount	//We don't want to heal wounds on irreparable organs.
		else if(!(parent_organ.brute_ratio < 1 || (parent_organ.organ_flags & ORGAN_FLAG_HEALS_OVERKILL)))
			return amount
