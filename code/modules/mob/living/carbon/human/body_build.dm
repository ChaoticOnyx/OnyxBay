...
/datum/body_build
	...
	var/list/clothing_icons = list(
		...
		"slot_ear"       = 'icons/inv_slots/ears/mob.dmi',
		...
		"slot_s_store"   = 'icons/mob/onmob/belt_mirror.dmi',
...
/datum/body_build/proc/get_mob_icon(slot, icon_state)
	...
	if(!slot in default_onmob_slots)
		log_and_message_admins("noticed shitty missing slot:[slot] and icon_state:[icon_state]!!!")
	...
	for(var/datum/body_build/BB in list(src, default_body_build))
		switch(slot)
			if(slot_handcuffed_str || slot == slot_legcuffed_str)
				I = BB.misk_icon
			if(slot_l_ear_str || slot_l_ear_str)
				I = BB.clothing_icons[slot_ear_str]
			else
...
/datum/body_build/slim
	...
	clothing_icons       = list(
		...
		"slot_ear"       = 'icons/inv_slots/ears/mob_slim.dmi',
		...
		"slot_s_store"   = 'icons/mob/onmob/belt_mirror_slim.dmi',
