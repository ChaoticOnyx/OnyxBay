/mob/living/carbon/human/proc/change_appearance(flags = APPEARANCE_ALL_HAIR, location = src, mob/user = src, list/species_blacklist = list(), datum/topic_state/state = GLOB.default_state)
	var/datum/nano_module/appearance_changer/AC = new(location, src, species_blacklist)
	AC.flags = flags
...
/mob/living/carbon/human/proc/generate_valid_species(list/blacklist = list())
