...
mob/living/carbon/human/proc/xeno_infest(mob/living/carbon/human/M as mob in oview())
	...
	if(M.species.name == "Xenophage" || !isnull(M.internal_organs_by_name["hive node"]))
	...
	if(M.species.name == "Xenophage" || !isnull(M.internal_organs_by_name["hive node"]) || !affecting || BP_IS_ROBOTIC(affecting))
		return
