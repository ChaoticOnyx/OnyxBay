...
/datum/species/xenos/handle_environment_special(mob/living/carbon/human/H)
	...
	if(environment.gas["plasma"] > 0 || locate(/obj/effect/alien/weeds) in T)
		if(!regenerate(H))
			...
			H.nutrition = max(H.nutrition+5, 400) // TODO: Come up with something better like eating humans or who tf knows what; for now it's still better than a horde of slowed-down hungry aliens
	..()
