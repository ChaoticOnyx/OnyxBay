	...
	if(opened)//Cover is open
		...
		if(wiresexposed)
			...
			if(prob(50))
				...
				if(remotable)
					log_game("[key_name(user)] tried to emag AI-controlled cyborg [key_name(src)].  Weapons unlocked.")
					...
					sleep(5)
					to_chat(src, SPAN("danger", "Attempting to overwrite law_synchronisation.ntb..."))
					sleep(5)
