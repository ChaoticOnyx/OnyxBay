...
/mob/living/silicon/robot/death(gibbed, ai_release, deathmessage, show_dead_message)
	if(camera)
		camera.status = 0
	if(module)
	...
	if(!ai_release)
		...
		..(gibbed,"emits a quiet beep and becomes motionless, its eyes suddenly darkening.", "You have released control over [src].")
