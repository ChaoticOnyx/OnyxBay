...
/obj/item/device/soulstone/examine(mob/user)
	...
	if(full == SOULSTONE_EMPTY)
		. = to_chat_or_concat(., user, "The shard still flickers with a fraction of the full artifact's power, but it needs to be filled with the essence of someone's life before it can be used.")
	if(full == SOULSTONE_ESSENCE)
		. = to_chat_or_concat(., user,"The shard has gone transparent, a seeming window into a dimension of unspeakable horror.")
	if(full == SOULSTONE_CRACKED)
		. = to_chat_or_concat(., user, "This one is cracked and useless.")
