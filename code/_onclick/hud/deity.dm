/mob/living/deity
	...
	var/obj/screen/deity_inv_background
...
/datum/hud/deity
	var/obj/screen/deity_current_building/selected
	var/obj/screen/deity_building_cost/cost
	var/obj/screen/deity_resource/followers
	var/obj/screen/deity_resource/buildings
...
/datum/hud/deity/FinalizeInstantiation(ui_style = 'icons/hud/deity.dmi')
	...
	var/obj/screen/deity_current_building/ccb = new()

	static_inventory += ccb
	selected = ccb

	var/obj/screen/deity_building_cost/cbc = new()
	...
	for(var/i in 1 to 4)
		var/obj/screen/deity_resource/res = new()
...
/datum/hud/deity/proc/update_resources(list/prints)
	for(var/i in 1 to prints.len)
		var/obj/screen/deity_resource/res = resources[i]
		var/p = prints[i]
...
/obj/screen/deity_current_building
	name = "Building"
...
/obj/screen/deity_current_building/proc/update_to_hud(datum/deity_power/selected_power)
	ClearOverlays()
	if(selected_power)
...
/obj/screen/deity_building_cost
	name = "Cost"
	screen_loc = "WEST:8,SOUTH:8"
	icon = 'icons/hud/deity_big.dmi'
	icon_state = "cost"
...
/obj/screen/deity_building_cost/proc/update_to_cost(list/cost)
	var/list/dat = list()
	dat += {"
...
/obj/screen/deity_resource/proc/update_resource(datum/deity_resource/res)
