...
/obj/item/attack_tk(mob/user)
	...
	if((MUTATION_TK in user.mutations) && !user.get_clicking_hand()) // both should already be true to get here
		var/obj/item/tk_grab/O = new(src)
		user.put_in_clicking_hand(O)
		O.host = user
		O.focus_object(src)
	else
		warning("Strange attack_tk(): TK([MUTATION_TK in user.mutations]) empty hand([!user.get_clicking_hand()])")
	return
