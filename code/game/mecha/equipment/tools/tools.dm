...
/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_clamp/action(atom/target)
	...
	if(istype(target,/obj))
		...
		if(istype(O, /obj/machinery/power/supermatter))
			occupant_message(SPAN_WARNING("Warning: Safety systems prevent the loading of [target] into the cargo compartment."))
			return
...
/obj/item/mecha_parts/mecha_equipment/armor_booster/attach(obj/mecha/M)
	activate_boost()
	. = ..()
...
/obj/item/mecha_parts/mecha_equipment/repair_droid/Initialize()
...
/obj/item/mecha_parts/mecha_equipment/tesla_energy_relay/Initialize()
...
/obj/mecha/proc/move_inside_passenger()
	...
	if(iscarbon(user))
		...
		if(C.handcuffed)
			to_chat(C, SPAN_DANGER("Kinda hard to climb in while handcuffed don't you think?"))
			return

	for(var/mob/living/carbon/slime/M in range(1, user))
		if(M.Victim == user)
			to_chat(M, SPAN_DANGER("You're too busy getting your life sucked out of you."))
			return
