...
/obj/machinery/sparker/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		...
		disable = !disable
		if(disable)
			user.visible_message(SPAN("warning", "[user] has disabled the [src]!"), SPAN("warning", "You disable the connection to the [src]."))
		else if(!disable)
			user.visible_message(SPAN("warning", "[user] has reconnected the [src]!"), SPAN("warning", "You fix the connection to the [src]."))
		update_icon()
	else
		..()
