...
/obj/machinery/requests_console/Topic(href, href_list)
	...
	if(reject_bad_text(href_list["write"]))
		...
		var/new_message = cp1251_to_utf8(sanitize(input_utf8("Write your message:", "Awaiting Input", "")))
		if(new_message)
			message = new_message
			screen = RCS_MESSAUTH
	...
	if( href_list["department"] && message )
		...
		if(pass)
			...
			audible_message(text("[icon2html(src, viewers(src), realsize=FALSE)] *The Requests Console beeps: 'NOTICE: No server detected!'"),,4)
