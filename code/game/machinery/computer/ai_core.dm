#define AI_STAGE_FRAME      0
#define AI_STAGE_CIRCUIT    1
#define AI_STAGE_CABLE      2
#define AI_STAGE_GLASS      3
#define AI_STAGE_CONNECTION 4
...
/obj/structure/AIcore
	...
	var/obj/item/organ/internal/mastermind/mmi/brain = null
	var/authorized = FALSE

/obj/structure/AIcore/emag_act(remaining_charges, mob/user, emag_source)
	if(!authorized)
		playsound(loc, 'sound/effects/computer_emag.ogg', 25)
		to_chat(user, SPAN("warning", "You swipe [emag_source] at [src] and jury rig it into the systems of [GLOB.using_map.full_name]!"))
		authorized = TRUE
		return 1
	. = ..()
	...
	switch(state)
		...
		if(AI_STAGE_CABLE)
			...
			if(isCoil(P))
				...
				playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
				if(do_after(user, 20, src) && state == AI_STAGE_CABLE)
					if(C.use(5))
						state = AI_STAGE_GLASS
						icon_state = "3"
						to_chat(user, SPAN("notice", "You have added cables to the frame."))
				return
		if(AI_STAGE_GLASS)
			if(isWirecutter(P))
				if(brain)
					to_chat(user, SPAN("notice", "Get that brain out of there first"))
				else
			...
			if(istype(P, /obj/item/stack/material) && P.get_material_name() == MATERIAL_REINFORCED_GLASS)
				...
				playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
				if(do_after(user, 20, src) && state == AI_STAGE_GLASS)
					if(RG.use(2))
						to_chat(user, SPAN("notice", "You put in the glass panel."))
						state = AI_STAGE_CONNECTION
						icon_state = "4"
			...
			if(!brain && (istype(P, /obj/item/organ/internal/mastermind/mmi) || istype(P, /obj/item/organ/internal/mastermind/posibrain)))
				var/mob/living/carbon/brain/B
				if(istype(P, /obj/item/organ/internal/mastermind/mmi))
					var/obj/item/organ/internal/mastermind/mmi/M = P
					B = M.brainmob
				else
					var/obj/item/organ/internal/mastermind/posibrain/PB = P
					B = PB.brainmob
				if(!B)
		...
		if(AI_STAGE_CONNECTION)
			...
			if(isScrewdriver(P))
				if(!authorized)
					to_chat(user, SPAN("warning", "Core fails to connect to the systems of [GLOB.using_map.full_name]!"))
					return
...
#undef AI_STAGE_FRAME
#undef AI_STAGE_CIRCUIT
#undef AI_STAGE_CABLE
#undef AI_STAGE_GLASS
#undef AI_STAGE_CONNECTION
