...
/obj/machinery/abductor/gland_dispenser/tgui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "GlandDispenser", name)
		ui.open()

/obj/machinery/abductor/gland_dispenser/tgui_data(mob/user)
	var/list/data = list()
	data["glands"] = list()
...
/obj/machinery/abductor/gland_dispenser/tgui_act(action, list/params)
	. = ..()
	if(.)
		return
