...
/mob/proc/changeling_division()
	...
	if(MUTATION_HUSK in T.mutations)
		to_chat(src, SPAN(LANGUAGE_LING, "This creature's DNA is ruined!"))
		return
	...
	if(!B || B.status == DEAD)
		to_chat(src, SPAN(LANGUAGE_LING, "[T] is dead. We need a living creature to divide."))
		return
	...
	for(var/stage = 1 to 3)
		...
		if(!do_mob(src, T, 150))
			to_chat(src, SPAN(LANGUAGE_LING, "Transfusion of new core into [T] has been interrupted!"))
			changeling.isabsorbing = FALSE
			return

	to_chat(src, SPAN(LANGUAGE_LING, "We successfully transfused new core into [T]!"))
	src.visible_message(SPAN("danger", "[src] transfused something into [T] through their proboscis!"))
	to_chat(T, SPAN(LANGUAGE_LING, "You... We... Feel like something new..."))
