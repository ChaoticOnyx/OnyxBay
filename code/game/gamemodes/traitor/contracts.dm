...
/datum/antag_contract/brig/New(datum/contract_organization/contract_organization, reason, datum/mind/target, selected_time)
	organization = contract_organization
	create_contract(reason, target, selected_time)
	..()

/datum/antag_contract/brig/generate_antag_reasons()
	..()
	reason_list[MODE_TRAITOR] = list("their loyalty to the Syndicate seems to be lost, teach them a lesson", 2, 40)
	reason_list[MODE_ERT] = list("they want to get bustle on this object via this target", 3, 80)
	ban_non_crew_antag()

/datum/antag_contract/brig/create_contract(Creason, datum/mind/target, selected_time)
	generate_antag_reasons()
	if(!Creason)
		reason = pick("they want to teach the lesson to target",
		"they want to set a lesson to target",
		"they want to show the target corruption of [GLOB.using_map.company_name]'s officials'")
	else
		reason = Creason
	...
	if(!target)
		...
		// Don't target the same player twice
		for(var/datum/antag_contract/brig/C in GLOB.all_contracts)
			candidates -= C.target_mind
	...
	if(selected_time)
		...
		time = rand(10, 20)
	create_explain_text("have [H.real_name], the [target_mind.assigned_role] brigged for [time] minutes.")
	time = time MINUTES

/datum/antag_contract/brig/can_place()
	return ..() && target_mind

/datum/antag_contract/brig/place()
	..()
	brig_timer()

/datum/antag_contract/brig/proc/brig_timer()
	if(completed)
		return
	if(target_mind && target_mind.current && target_mind.current.stat != DEAD)
		if(target_mind.is_brigged(time))
			return // finnaly brigged
	addtimer(CALLBACK(src, .proc/brig_timer), 1 MINUTE) // 1 minute will be fine, I guess.

/datum/antag_contract/brig/on_confirm_action()
	. = ..()
	if(. && accepted_by_uplink)
		if(target_mind && target_mind.current && target_mind.current.stat != DEAD)
			if(target_mind.is_brigged(time))
...
/datum/antag_contract/brig/on_mob_despawned(datum/mind/M)
	if(M == target_mind)
		remove()
