...
/datum/antag_contract/item/assassinate/check_contents(list/contents)
	var/obj/item/organ/internal/mastermind/brain/_brain = brain?.resolve()
	var/obj/item/organ/internal/mastermind/mmi/MMI = _brain?.loc
...
/datum/antag_contract/item/assassinate/complete(obj/item/device/uplink/close_uplink)
	...
	if((istype(_H) && !_H.stat && _H.mind) || (istype(_brain?.loc, /obj/item/organ/internal/mastermind/mmi) && !target_detected_in_STD))
		if(M)
