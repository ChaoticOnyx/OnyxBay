...
/obj/item/weapon/book/tome/examine(mob/user as mob)
	. = ..()
	if(!iscultist(user))
		. = to_chat_or_concat(., user, "An old, dusty tome with frayed edges and a sinister looking cover.")
	else
		. = to_chat_or_concat(., user, "The scriptures of Nar-Sie, The One Who Sees, The Geometer of Blood. Contains the details of every ritual his followers could think of. Most of these are useless, though.")
