...
/mob/living/carbon/human/proc/vampire_drain_blood()
	...
	if (!istype(G))
		...
		return
	if(!G.can_absorb())
		to_chat(src, SPAN("warning", "You must have a tighter grip on victim to drain their blood."))
		return
	...
	if(blocked)
		to_chat(src, SPAN("warning", "\The [blocked] is in the way of your fangs!"))
		return
	...
	to_chat(T, SPAN("warning", "You feel yourself falling into a pleasant dream, from which even a smile appeared on your face."))
	T.paralysis = 3400
	...
	while (do_mob(src, T, 50))
		...
		if (blood_drained >= 115)
			to_chat(src, SPAN("danger", "You can't drink even more blood!"))
			break
		...
		if (blood_total != vampire.blood_total)
			...
			if (blood_usable != vampire.blood_usable)
				...
				update_msg += SPAN("notice", ".")
			to_chat(src, update_msg)

		if (blood_drained >= 70 && blood_drained < 85)
			to_chat(src, SPAN("warning", "You have enough amount of drained blood."))
	...
	visible_message(SPAN("danger", "[src.name] stops biting [T.name]'s neck!"), SPAN("notice", "[endsuckmsg]"))
	if(target_aware)
		T.paralysis = 0
		if(T.stat != DEAD && vampire.stealth)
			to_chat(T, SPAN("warning", "You remember nothing about the cause of blacked out. Instead, you simply remember having a pleasant encounter with [src.name]"))
		else if(T.stat != DEAD)
			to_chat(T, SPAN("warning", "You remember everything that happened. Remember how blood was sucked from your neck. It gave you pleasure, like a pleasant dream. You feel great. How you react to [src.name]'s actions is up to you."))
...
/mob/living/carbon/human/proc/vampire_glare()
	...
	if (eyecheck() > FLASH_PROTECTION_NONE)
		to_chat(src, SPAN("warning", "You can't do that, because no one will see the light of your eyes!"))
		return

	visible_message(SPAN("danger", "[src.name]'s eyes emit a blinding flash"))
	...
	for (var/mob/living/carbon/human/H in view(2))
		...
		H.stuttering = 20
		H.confused = 10
		to_chat(H, SPAN("danger", "You are blinded by [src]'s glare!"))
		H.flash_eyes()
		victims += H
...
/mob/living/carbon/human/proc/vampire_hypnotise()
	...
	if (eyecheck() > FLASH_PROTECTION_NONE)
		to_chat(src, SPAN("warning", "You can't do that, because no one will see your eyes!"))
		return
	...
	for (var/mob/living/carbon/human/H in view(3))
		...
		victims += H
	if (!victims.len)
		to_chat(src, SPAN("warning", "No suitable targets."))
		return
	...
	to_chat(src, SPAN("notice", "You begin peering into [T.name]'s mind, looking for a way to render them useless."))

	if (do_mob(src, T, 50))
		to_chat(src, SPAN("danger", "You dominate [T.name]'s mind and render them temporarily powerless to resist"))
		to_chat(T, SPAN("danger", "You are captivated by [src.name]'s gaze, and find yourself unable to move or even speak."))
		T.Weaken(25)
		T.Stun(25)
		T.silent += 30
		...
		to_chat(src, SPAN("warning", "You broke your gaze."))
...
/mob/living/carbon/human/proc/vampire_veilstep(turf/simulated/floor/T in view(7))
	...
	if (!T || T.density || T.contains_dense_objects())
		to_chat(src, SPAN("warning", "You cannot do that."))
		return
	...
	if (!vampire)
		return
	if (!istype(loc, /turf))
		to_chat(src, SPAN("warning", "You cannot teleport out of your current location."))
		return
	if (T.z != src.z || get_dist(T, get_turf(src)) > world.view)
		to_chat(src, SPAN("warning", "Your powers are not capable of taking you that far."))
		return
	if (T.get_lumcount() > 0.1)
		// Too bright, cannot jump into.
		to_chat(src, SPAN("warning", "The destination is too bright."))
		return
...
/obj/effect/dummy/veil_walk/relaymove(mob/user, direction)
	...
	if (new_loc.turf_flags & TURF_FLAG_NOJAUNT || istype(new_loc.loc, /area/chapel))
		to_chat(usr, SPAN("warning", "Some strange aura is blocking the way!"))
		return
...
/obj/effect/dummy/veil_walk/Process()
	if (owner_mob.stat)
		if (owner_mob.stat == 1)
			to_chat(owner_mob, SPAN("warning", "You cannot maintain this form while unconcious."))
		...
		switch (warning_level)
			if (0)
				if (owner_vampire.blood_usable <= 5 * 20)
					to_chat(owner_mob, SPAN("notice", "Your pool of blood is diminishing. You cannot stay in the veil for too long."))
					warning_level = 1
			if (1)
				if (owner_vampire.blood_usable <= 5 * 10)
					to_chat(owner_mob, SPAN("warning", "You will be ejected from the veil soon, as your pool of blood is running dry."))
					warning_level = 2
			if (2)
				if (owner_vampire.blood_usable <= 5 * 5)
					to_chat(owner_mob, SPAN("danger", "You cannot sustain this form for any longer!"))
					warning_level = 3
	else
		deactivate()
...
/obj/effect/dummy/veil_walk/proc/kick_unconcious()
	if (owner_mob && owner_mob.stat == 1)
		to_chat(owner_mob, SPAN("danger", "You are ejected from the Veil."))
		deactivate()
		return
...
/mob/living/carbon/human/proc/vampire_bloodheal()
	...
	if (vampire.status & VAMP_HEALING)
		...
		to_chat(src, SPAN("warning", "You do not have enough usable blood. 15 needed."))
		return

	vampire.status |= VAMP_HEALING
	to_chat(src, SPAN("notice", "You begin the process of blood healing. Do not move, and ensure that you are not interrupted."))

	log_and_message_admins("activated blood heal.")

	while (do_after(src, 20, 0))
		if (!(vampire.status & VAMP_HEALING))
			to_chat(src, SPAN("warning", "Your concentration is broken! You are no longer regenerating!"))
			break
		...
		if (prob(20))
			visible_message(SPAN("danger", "[pick(emotes_lookers)]"), SPAN("notice", "[pick(emotes_self)]"))

		if (vampire.blood_usable <= blood_used)
			vampire.blood_usable = 0
			vampire.status &= ~VAMP_HEALING
			to_chat(src, SPAN("warning", "You ran out of blood, and are unable to continue!"))
			break
		else if (!blood_used)
			vampire.status &= ~VAMP_HEALING
			to_chat(src, SPAN("notice", "Your body has finished healing. You are ready to continue."))
			break
...
/mob/living/carbon/human/proc/vampire_dominate()
	...
	if (!(vampire.status & VAMP_FULLPOWER))
		to_chat(src, SPAN("notice", "You begin peering into [T]'s mind, looking for a way to gain control."))

		if (!do_mob(src, T, 50))
			to_chat(src, SPAN("warning", "Your concentration is broken!"))
			return

		to_chat(src, SPAN("notice", "You succeed in dominating [T]'s mind. They are yours to command."))
	else
		to_chat(src, SPAN("notice", "You instantly dominate [T]'s mind, forcing them to obey your command."))
	...
	to_chat(src, SPAN("notice", "You command [T], and they will obey."))
	emote("me", 1, "whispers.")
...
/mob/living/carbon/human/proc/vampire_enthrall()
	...
	var/obj/item/grab/G = get_active_hand()
	if (!istype(G))
		to_chat(src, SPAN("warning", "You must be grabbing a victim in your active hand to enthrall them."))
		return
	if(!G.can_absorb())
		to_chat(src, SPAN("warning", "You must have a tighter grip on victim to enthrall them."))
		return

	var/mob/living/carbon/human/T = G.affecting
	if(is_mechanical(T))
		to_chat(src, SPAN("warning", "[T] is not a creature you can enthrall."))
		return
	if (!istype(T))
		to_chat(src, SPAN("warning", "[T] is not a creature you can enthrall."))
		return
	if (!vampire_can_affect_target(T, 1, 1))
		return
	if (!T.client || !T.mind)
		to_chat(src, SPAN("warning", "[T]'s mind is empty and useless. They cannot be forced into a blood bond."))
		return
	if (vampire.status & VAMP_DRAINING)
		to_chat(src, SPAN("warning", "Your fangs are already sunk into a victim's neck!"))
		return

	visible_message(SPAN("danger", "[src] tears the flesh on their wrist, and holds it up to [T]. In a gruesome display, [T] starts lapping up the blood that's oozing from the fresh wound."), SPAN("warning", "You inflict a wound upon yourself, and force them to drink your blood, thus starting the conversion process"))
	to_chat(T, SPAN("warning", "You feel an irresistable desire to drink the blood pooling out of [src]'s wound. Against your better judgement, you give in and start doing so."))

	if (!do_mob(src, T, 50))
		visible_message(SPAN("warning", "[src] yanks away their hand from [T]'s mouth as they're interrupted, the wound quickly sealing itself!"), SPAN("danger", "You are interrupted!"))
		return

	to_chat(T, SPAN("danger", "Your mind blanks as you finish feeding from [src]'s wrist."))
	GLOB.thralls.add_antagonist(T.mind, 1, 1, 0, 1, 1)

	T.mind.vampire.master = src
	vampire.thralls += T
	to_chat(T, SPAN("notice", "You have been forced into a blood bond by [T.mind.vampire.master], and are thus their thrall. While a thrall may feel a myriad of emotions towards their master, ranging from fear, to hate, to love; the supernatural bond between them still forces the thrall to obey their master, and to listen to the master's commands.<br><br>You must obey your master's orders, you must protect them, you cannot harm them."))
	to_chat(src, SPAN("notice", "You have completed the thralling process. They are now your slave and will obey your commands."))
...
/mob/living/carbon/human/proc/vampire_presence()
	...
	if (vampire.status & VAMP_PRESENCE)
		...
		to_chat(src, SPAN("warning", "You are no longer influencing those weak of mind."))
		return
	else if (vampire.blood_usable < 15)
		to_chat(src, SPAN("warning", "You do not have enough usable blood. 15 needed."))
		return

	to_chat(src, SPAN("notice", "You begin passively influencing the weak minded."))
	vampire.status |= VAMP_PRESENCE
	...
	while (vampire.status & VAMP_PRESENCE)
		...
		if (stat)
			to_chat(src, SPAN("warning", "You cannot influence people around you while [stat == 1 ? "unconcious" : "dead"]."))
			vampire.status &= ~VAMP_PRESENCE
			break
...
/mob/living/carbon/human/proc/vampire_touch_of_life()
	...
	var/obj/item/grab/G = get_active_hand()
	if (!istype(G))
		to_chat(src, SPAN("warning", "You must be grabbing a victim in your active hand to touch them."))
		return

	var/mob/living/carbon/human/T = G.affecting
	if (T.species.species_flags & SPECIES_FLAG_NO_BLOOD)
		to_chat(src, SPAN("warning", "[T] has no blood and can not be affected by your powers!"))
		return
...
/mob/living/carbon/human/proc/vampire_embrace()
	...
	if (!vampire_can_affect_target(T, ignore_thrall = TRUE))
		return
	if (!T.client)
		to_chat(src, SPAN("warning", "[T] is a mindless husk. The Veil has no purpose for them."))
		return
	if (T.stat == 2)
		to_chat(src, SPAN("warning", "[T]'s body is broken and damaged beyond salvation. You have no use for them."))
		return
	if (T.species.species_flags & SPECIES_FLAG_NO_BLOOD)
		to_chat(src, SPAN("warning", "[T] has no blood and can not be affected by your powers!"))
		return
	...
	if (T.mind.vampire)
		...
		if (draining_vamp.status & VAMP_ISTHRALL)
			...
			if (alert(src, choice_text, "Choices", "Yes", "No") == "No")
				to_chat(src, SPAN("notice", "[denial_response]"))
				return
			...
			to_chat(src, SPAN("warning", "You feel corruption running in [T]'s blood. Much like yourself, \he[T] is already a spawn of the Veil, and cannot be Embraced."))
			return
	...
	to_chat(src, SPAN("warning", "You have corrupted another mortal with the taint of the Veil. Beware: they will awaken hungry and maddened; not bound to any master."))
...
/mob/living/carbon/human/proc/grapple()
	...
	if (status_flags & LEAPING)
		return
	if (incapacitated())
		to_chat(src, SPAN("warning", "You cannot lean in your current state."))
		return
	...
	if (!targets.len)
		to_chat(src, SPAN("warning", "No valid targets visible or in range."))
		return

	var/mob/living/carbon/human/T = pick(targets)

	visible_message(SPAN("danger", "[src] leaps at [T]!"))
	src.drop_item()
	...
	src.visible_message(SPAN("warning", "[src] seizes [T] aggressively!"))
