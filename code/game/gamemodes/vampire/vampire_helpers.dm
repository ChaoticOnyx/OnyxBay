...
/mob/proc/check_vampire_upgrade()
	...
	if (!(vampire.status & VAMP_FULLPOWER) && vampire.blood_total >= 650)
		...
		to_chat(src, SPAN("notice", "You've gained full power. Some abilities now have bonus functionality, or work faster."))
...
/mob/proc/vampire_power(required_blood = 0, max_stat = 0, ignore_holder = 0, disrupt_healing = 1,required_vampire_blood = 0)
	...
	if (vampire.holder && !ignore_holder)
		to_chat(src, SPAN("warning", "You cannot use this power while walking through the Veil."))
		return
	if (stat > max_stat)
		to_chat(src, SPAN("warning", "You are incapacitated."))
		return
	if (required_blood > vampire.blood_usable)
		to_chat(src, SPAN("warning", "You do not have enough usable blood. [required_blood] needed."))
		return
...
/mob/proc/vampire_can_affect_target(mob/living/carbon/human/T, notify = 1, account_loyalty_implant = 0, ignore_thrall = FALSE)
	...
	if (T.mind)
		if (T.mind.assigned_role == "Chaplain")
			if (notify)
				to_chat(src, SPAN("warning", "Your connection with the Veil is not strong enough to affect a man as devout as them."))
			return FALSE
		else if (T.mind.vampire && (!(T.mind.vampire.status & VAMP_ISTHRALL) || ((T.mind.vampire.status & VAMP_ISTHRALL) && !ignore_thrall)))
			if (notify)
				to_chat(src, SPAN("warning", "You lack the power required to affect another creature of the Veil."))
			return FALSE

	if (is_mechanical(T))
		if (notify)
			to_chat(src, SPAN("warning", "You lack the power interact with mechanical constructs."))
		return FALSE
	if(is_special_character(T) && (!(T.mind.vampire.status & VAMP_ISTHRALL)))
		if (notify)
			to_chat(src, SPAN("warning", "\The [T]'s mind is too strong to be affected by our powers!"))
		return FALSE
	if (account_loyalty_implant)
		for (var/obj/item/weapon/implant/loyalty/I in T)
			if (I.implanted)
				if (notify)
					to_chat(src, SPAN("warning", "You feel that [T]'s mind is protected from our powers."))
				return FALSE
