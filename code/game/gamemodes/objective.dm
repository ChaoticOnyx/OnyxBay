		...
		if("a functional AI")
			for(var/mob/living/silicon/ai/ai in SSmobs.mob_list)
				...
				if(owner.current.contains(ai) || (T && is_type_in_list(T.loc, GLOB.using_map.post_round_safe_areas)))
					return 1
		if("a bluespace rift generator")
			for(var/obj/I in all_items)
				if(istype(I, steal_target))
					return 1
				if(is_type_in_list(I, list(/obj/item/device/electronic_assembly, /obj/item/electronic_assembly_shell/hand_teleporter)))
					if(locate(/obj/item/integrated_circuit/manipulation/bluespace_rift) in I.contents)
						return 1
		else
