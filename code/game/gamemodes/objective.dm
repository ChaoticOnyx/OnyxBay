...
/datum/objective/anti_revolution/brig/check_completion()
	...
	if(target?.current)
		if(target.current.stat == DEAD)
			return FALSE
		if(target.brigged_for_max >= 10 MINUTES)
			return TRUE
		return FALSE
	return FALSE
...
/datum/objective/heist/kidnap/check_completion()
	if(target && target.current)
		...
		if(is_type_in_list(get_area(target), list(/area/skipjack_station/base, /area/skipjack_station/start)))
			return TRUE
	else
		return FALSE
