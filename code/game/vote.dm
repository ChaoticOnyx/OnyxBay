...
/mob/verb/vote()
	...
	if(vote.voting)
		...
		if(!vote.canvote())		// not time to vote yet
			...
			if(config.allow_vote_mode&& ticker && ticker.current_state == 1) //stop people from starting game mode votes during rounds, since they always restart the game if there is a winner
