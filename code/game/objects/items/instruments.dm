//рома блять допиши это все


/obj/item/instrument/guitar
	name = "guitar"
	desc = "Wow, is this still working?"
...
/obj/item/instrument/guitar/proc/playnote(var/note as text)
//	log_debug("Note: [note]")
...
/obj/item/instrument/guitar/proc/playsong()
	do
		var/cur_oct[7]
		var/cur_acc[7]
		for(var/i = 1 to 7)
...
/obj/item/instrument/guitar/Topic(href, href_list)
	if(..())
		return 1
...
/obj/item/instrument/violin/proc/playnote(var/note as text)
//	log_debug("Note: [note]")
...
/obj/item/instrument/violin/proc/playsong()
	do
		var/cur_oct[7]
		var/cur_acc[7]
		for(var/i = 1 to 7)
...
/obj/item/instrument/violin/attack_self(mob/user as mob)
	if(!isliving(user) || user.stat || user.restrained() || user.lying)	return
	user.set_machine(src)
...
/obj/item/instrument/violin/Topic(href, href_list)
	if(..())
		return 1
	if(!in_range(src, usr) || issilicon(usr) || !isliving(usr) || !usr.canmove || usr.restrained())
...
/obj/item/instrument/drum/proc/playnote(var/note as text)
//	log_debug("Note: [note]")
	...
	Tried doing it dynamically at first but its more trouble than its worth. Would have saved many lines tho.*/
	switch(note)
		if("Ab3") soundfile = 'sound/drum/Ab3.mid'
		if("Ab4") soundfile = 'sound/drum/Ab4.mid'
		if("Ab5") soundfile = 'sound/drum/Ab5.mid'
		if("Ab6") soundfile = 'sound/drum/Ab6.mid'
		if("An3") soundfile = 'sound/drum/An3.mid'
		if("An4") soundfile = 'sound/drum/An4.mid'
		if("An5") soundfile = 'sound/drum/An5.mid'
		if("An6") soundfile = 'sound/drum/An6.mid'
		if("Bb3") soundfile = 'sound/drum/Bb3.mid'
		if("Bb4") soundfile = 'sound/drum/Bb4.mid'
		if("Bb5") soundfile = 'sound/drum/Bb5.mid'
		if("Bb6") soundfile = 'sound/drum/Bb6.mid'
		if("Bn3") soundfile = 'sound/drum/Bn3.mid'
		if("Bn4") soundfile = 'sound/drum/Bn4.mid'
		if("Bn5") soundfile = 'sound/drum/Bn5.mid'
		if("Bn6") soundfile = 'sound/drum/Bn6.mid'
		if("Cb4") soundfile = 'sound/drum/Cn4.mid'
		if("Cb5") soundfile = 'sound/drum/Cn4.mid'
		if("Cb6") soundfile = 'sound/drum/Cn5.mid'
		if("Cb7") soundfile = 'sound/drum/Cn6.mid'
		if("Cn4") soundfile = 'sound/drum/Cn4.mid'
		if("Cn5") soundfile = 'sound/drum/Cn5.mid'
		if("Cn6") soundfile = 'sound/drum/Cn6.mid'
		if("Db4") soundfile = 'sound/drum/Db4.mid'
		if("Db5") soundfile = 'sound/drum/Db5.mid'
		if("Db6") soundfile = 'sound/drum/Db6.mid'
		if("Dn4") soundfile = 'sound/drum/Dn4.mid'
		if("Dn5") soundfile = 'sound/drum/Dn5.mid'
		if("Dn6") soundfile = 'sound/drum/Dn6.mid'
		if("Eb4") soundfile = 'sound/drum/Eb4.mid'
		if("Eb5") soundfile = 'sound/drum/Eb5.mid'
		if("Eb6") soundfile = 'sound/drum/Eb6.mid'
		if("En3") soundfile = 'sound/drum/En3.mid'
		if("En4") soundfile = 'sound/drum/En4.mid'
		if("En5") soundfile = 'sound/drum/En5.mid'
		if("En6") soundfile = 'sound/drum/En6.mid'
		if("Fb3") soundfile = 'sound/drum/Fn3.mid'
		if("Fb4") soundfile = 'sound/drum/Fn4.mid'
		if("Fb5") soundfile = 'sound/drum/Fn5.mid'
		if("Fb6") soundfile = 'sound/drum/Fn6.mid'
		if("Fn3") soundfile = 'sound/drum/Fn3.mid'
		if("Fn4") soundfile = 'sound/drum/Fn4.mid'
		if("Fn5") soundfile = 'sound/drum/Fn5.mid'
		if("Fn6") soundfile = 'sound/drum/Fn6.mid'
		if("Gb3") soundfile = 'sound/drum/Gb3.mid'
		if("Gb4") soundfile = 'sound/drum/Gb4.mid'
		if("Gb5") soundfile = 'sound/drum/Gb5.mid'
		if("Gb6") soundfile = 'sound/drum/Gb6.mid'
		if("Gn3") soundfile = 'sound/drum/Gn3.mid'
		if("Gn4") soundfile = 'sound/drum/Gn4.mid'
		if("Gn5") soundfile = 'sound/drum/Gn5.mid'
		if("Gn6") soundfile = 'sound/drum/Gn6.mid'
		else		return

	sound_to(hearers(15, get_turf(src)), sound(soundfile))

/obj/item/instrument/drum/proc/playsong()
	do
		var/cur_oct[7]
		var/cur_acc[7]
		for(var/i = 1 to 7)
...
//					log_debug("note: [note]")
					...
					if(!playing || !isliving(loc))//If the drum is playing, or isn't held by a person
						playing = 0
						return
					if(lentext(note) == 0)
						continue
...
/obj/item/instrument/drum/attack_self(mob/user as mob)
	if(!isliving(user) || user.stat || user.restrained() || user.lying)	return
	user.set_machine(src)

	var/dat = "<HEAD><TITLE>drum</TITLE></HEAD><BODY>"
	...
	if(!edit)
		...
		if(help)
			...
			dat += "<A href='?src=\ref[src];help=2'>Show Help</A><BR>"
	dat += "</BODY></HTML>"
	user << browse(dat, "window=drum;size=700x300")
	onclose(user, "drum")

/obj/item/instrument/drum/Topic(href, href_list)
	if(..())
		return 1
	if(!in_range(src, usr) || issilicon(usr) || !isliving(usr) || !usr.canmove || usr.restrained())
		usr << browse(null, "window=drum;size=700x300")
		onclose(usr, "drum")
		return
	...
	if(href_list["newsong"])
		...
		if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
			...
			var/newline = html_encode(input("Enter your line: ", "drum") as text|null)
			if(!newline)
				return
			if(song.lines.len > 50)
				return
			...
			var/num = round(text2num(href_list["modifyline"]),1)
			var/content = html_encode(input("Enter your line: ", "drum", song.lines[num]) as text|null)
			if(!content)
				return
			if(lentext(content) > 50)
				content = copytext(content, 1, 50)
...
/obj/item/instrument/harmonica
	name = "harmonica"
	desc = "Yuriy will be doot`, doot` will be Yuriy."
	icon = 'icons/obj/musician.dmi'
...
//	log_debug("Note: [note]")
	...
	switch(note)
		...
		if("An2") soundfile = 'sound/harmonica/An2.mid'
		...
		if("Cn7") soundfile = 'sound/harmonica/Cn7.mid'
...
//	log_debug("Note: [note]")
	...
	switch(note)
		...
		if("An2") soundfile = 'sound/saxophone/An2.mid'
