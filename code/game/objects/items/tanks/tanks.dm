...
/obj/item/tank/_examine_text(mob/user)
	...
	if(get_dist(src, user) <= 0)
		...
		if(air_contents.total_moles == 0)
			...
			var/celsius_temperature = convert_k2c(air_contents.temperature)
	...
	if(istype(W, /obj/item/device/assembly_holder))
		if(wired)
			...
			if(do_after(user, 50, src))
				...
				GLOB.bombers += "[key_name(user)] attached an assembly to a wired [src]. Temp: [convert_k2c(air_contents.temperature)]"
				message_admins("[key_name_admin(user)] attached an assembly to a wired [src]. Temp: [convert_k2c(air_contents.temperature)]")
		...
		if(WT.remove_fuel(1,user))
			if(!valve_welded)
				...
				if(do_after(user, 40,src))
					...
					GLOB.bombers += "[key_name(user)] attempted to weld a [src]. [convert_k2c(air_contents.temperature)]"
					message_admins("[key_name_admin(user)] attempted to weld a [src]. [convert_k2c(air_contents.temperature)]")
					if(WT.welding)
...
/obj/item/tank/proc/check_status()
	...
	if(pressure > TANK_FRAGMENT_PRESSURE)
		...
		if(integrity <= 0)
			...
			integrity-= 5
	else if((pressure > TANK_LEAK_PRESSURE) || convert_k2c(air_contents.temperature) > failure_temp)
