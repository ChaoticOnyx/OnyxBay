...
/obj/item/vortex_manipulator/attack_self(mob/user)
	if(cover_open)
		user.set_machine(src)
...
/obj/item/vortex_manipulator/attackby(obj/item/W, mob/user)
	if(cover_open)
...
/obj/item/vortex_manipulator/Topic(href, href_list)
...
/obj/item/vortex_manipulator/emp_act(severity)
...
/obj/item/vortex_manipulator/afterattack(atom/A, mob/user, proximity)
...
/obj/item/vortex_manipulator/Initialize()
	. = ..()
	GLOB.vortex_manipulators += src

/obj/item/vortex_manipulator/Destroy()
	GLOB.vortex_manipulators -= src
	return ..()

/obj/item/vortex_manipulator/proc/self_activate(mob/living/carbon/human/user)
	if(!active)
		to_chat(user, SPAN_NOTICE("You attempt to activate Vortex Manipulator"))
		if(!timelord_mode)
			var/counter = 0
			for(var/obj/item/vortex_manipulator/VM in GLOB.vortex_manipulators)
			...
			if (counter > 3)
				...
				return
		active = 1
		unique_id = random_id(/obj/item/vortex_manipulator, 1111, 9999)
...
/obj/item/vortex_manipulator/proc/get_owner()
...
/obj/item/vortex_manipulator/proc/malfunction()
	if(timelord_mode)
		return
	var/vm_owner = get_owner()
	if(!ishuman(vm_owner))
...
/obj/item/vortex_manipulator/proc/deductcharge(chrgdeductamt)
	if(vcell)
...
/obj/item/vortex_manipulator/proc/phase_in(mob/M,turf/T)
	if(!M || !T)
		return
	playsound(T, 'sound/effects/phasein.ogg', 50, 1)
	anim(T,M,'icons/mob/mob.dmi',,"phasein",,M.dir)

/obj/item/vortex_manipulator/proc/phase_out(mob/M,turf/T)
	if(!M || !T)
		return
	playsound(T, 'sound/effects/phasein.ogg', 50, 1)
 ...
 */
/obj/item/vortex_manipulator/proc/localmassiverandom(mob/user)
	if(istype(vcell, /obj/item/cell/quantum))
		var/obj/item/cell/quantum/Q = vcell
		if(Q.partner)
			playsound(src, 'sound/effects/phasein.ogg', 50, 1)
			user.visible_message(SPAN_WARNING("The Vortex Manipulator turns into a potato!"))
			new /obj/item/cell/potato(get_turf(src))
			qdel(src)
			return
	...
	for(var/mob/M in range(5, temp_turf))
		...
		for(var/obj/item/vortex_manipulator/VM in M.contents)
			if(VM.active == 1)
				vortexchecktemp = 1
		if(!vortexchecktemp)
			localteleport(M, 1)
...
/obj/item/vortex_manipulator/proc/vortexannounce(mob/user, nonactive_announce = 0)
 ...
 */
/obj/item/vortex_manipulator/proc/localteleport(mob/user, malf_use, new_x = 0, new_y = 0)
	if(!active)
		malf_use = 1
 ...
 * Also teleports everyone aggressively grabbed by user.
 */
/obj/item/vortex_manipulator/proc/beaconteleport(mob/user, malf_use)
	if(!active)
		malf_use = 1
 ...
 * Also teleports everyone aggressively grabbed by user.
 */
/obj/item/vortex_manipulator/proc/areateleport(mob/user, malf_use)
	if(!active)
		malf_use = 1
