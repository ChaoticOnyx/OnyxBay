...
/obj/item/weapon/syndicate_uplink/attack_self(mob/user as mob)
	...
	if (src.selfdestruct)
		...
		if (src.temp)
			...
			dat += "<A href='byond://?src=\ref[src];imp_compress=1'>Compressed Matter Implant (with injector)</A> (5)<BR>"
			dat += "<A href='byond://?src=\ref[src];imp_vfac=1'>Viral Factory Implant</A> (5)<BR>"
			dat += "<A href='byond://?src=\ref[src];imp_explosive=1'>Explosive Implant (with injector)</A> (6)<BR>"
