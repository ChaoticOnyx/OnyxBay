...
/obj/item/tape_roll/attack(mob/living/carbon/human/H, mob/user)
	if(istype(H))
		if(user.zone_sel.selecting == BP_EYES)
			...
			if(!H || !src || !H.external_organs_by_name[BP_HEAD] || !H.has_eyes() || H.glasses || (H.head && (H.head.body_parts_covered & FACE)))
				return
			...
			// Repeat failure checks.
			if(!H || !src || !H.external_organs_by_name[BP_HEAD] || !H.check_has_mouth() || H.wear_mask || (H.head && (H.head.body_parts_covered & FACE)))
				return
