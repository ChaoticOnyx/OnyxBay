...
/obj/item/card/id/syndicate/afterattack(obj/item/O as obj, mob/user as mob, proximity)
	...
	if(istype(O, /obj/item/card/id))
		...
		if(player_is_antag(user.mind))
			to_chat(user, SPAN("notice", "The microscanner activates as you pass it over the ID, copying its access."))
...
/obj/item/card/id/syndicate/attack_self(mob/user)
	...
	if(!registered_user && register_user(user))
		to_chat(user, SPAN("notice", "The microscanner marks you as its owner, preventing others from accessing its internals."))
...
/obj/item/card/id/syndicate/proc/unset_registered_user(mob/user)
	if(!registered_user || (user && user != registered_user))
		return
	unregister_signal(registered_user, SIGNAL_QDELETING)
...
/obj/item/card/id/syndicate/Topic(href, href_list, datum/topic_state/state)
	...
	if(href_list["electronic_warfare"])
		...
		to_chat(user, SPAN("notice", "Electronic warfare [electronic_warfare ? "enabled" : "disabled"]."))
			...
			if("Age")
				...
				if(!isnull(new_age) && CanUseTopic(user, state))
					if(new_age < 0)
						...
						age = new_age
					to_chat(user, SPAN("notice", "Age has been set to '[age]'."))
					. = TRUE
			if("Appearance")
				...
				if(choice && CanUseTopic(user, state))
					...
					to_chat(usr, SPAN("notice", "Appearance changed to [choice]."))
					. = TRUE
			if("Assignment")
				...
				if(!isnull(new_job) && CanUseTopic(user, state))
					...
					to_chat(user, SPAN("notice", "Occupation changed to '[new_job]'."))
					update_name()
					. = TRUE
			if("Blood Type")
				...
				if(!isnull(new_blood_type) && CanUseTopic(user, state))
					...
					to_chat(user, SPAN("notice", "Blood type changed to '[new_blood_type]'."))
					. = TRUE
			if("DNA Hash")
				...
				if(!isnull(new_dna_hash) && CanUseTopic(user, state))
					...
					to_chat(user, SPAN("notice", "DNA hash changed to '[new_dna_hash]'."))
					. = TRUE
			if("Fingerprint Hash")
				...
				if(!isnull(new_fingerprint_hash) && CanUseTopic(user, state))
					...
					to_chat(user, SPAN("notice", "Fingerprint hash changed to '[new_fingerprint_hash]'."))
					. = TRUE
			if("Name")
				...
				if(!isnull(new_name) && CanUseTopic(user, state))
					...
					update_name()
					to_chat(user, SPAN("notice", "Name changed to '[new_name]'."))
					. = TRUE
			if("Photo")
				set_id_photo(user)
				to_chat(user, SPAN("notice", "Photo changed."))
				. = TRUE
			if("Sex")
				...
				if(!isnull(new_sex) && CanUseTopic(user, state))
					src.sex = new_sex
					to_chat(user, SPAN("notice", "Sex changed to '[new_sex]'."))
					. = TRUE
			if("Factory Reset")
				if(alert("This will factory reset the card, including access and owner. Continue?", "Factory Reset", "No", "Yes") == "Yes" && CanUseTopic(user, state))
					...
					unset_registered_user()
					sex = initial(sex)
					to_chat(user, SPAN("notice", "All information has been deleted from \the [src]."))
					. = TRUE
