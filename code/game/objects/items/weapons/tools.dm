...
/obj/item/weapon/weldingtool/examine(mob/user)
	if(..(user, 0))
		if(tank)
			to_chat(user, "[GUNCHAT_ICON(tank)] \The [tank] contains [get_fuel()]/[tank.max_fuel] units of fuel!")
		else
...
/obj/item/weapon/combitool/examine()
	...
	if(loc == usr && tools.len)
		...
		for(var/obj/item/tool in tools)
			to_chat(usr, "[GUNCHAT_ICON(tool)] - [tool.name][tools[current_tool]==tool?" (selected)":""]")
