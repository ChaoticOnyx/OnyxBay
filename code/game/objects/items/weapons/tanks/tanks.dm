...
/obj/item/weapon/tank/examine(mob/user)
	...
	if(.)
		...
		. = to_chat_or_concat(., user, "<span class='notice'>\The [src] feels [descriptive].</span>")

	if(proxyassembly.assembly || wired)
		. = to_chat_or_concat(., user, "<span class='warning'>It seems to have [wired? "some wires ": ""][wired && proxyassembly.assembly? "and ":""][proxyassembly.assembly ? "some sort of assembly ":""]attached to it.</span>")
	if(valve_welded)
		. = to_chat_or_concat(., user, "<span class='warning'>\The [src] emergency relief valve has been welded shut!</span>")
