			...
			for(var/dir in GLOB.cardinal)
				...
				if(T && T.density)
					...
					for(var/obj/structure/window/W in T)
						if(W.is_full_window || W.dir == GLOB.reverse_dir[dir])
							possible_dirs += dir
			if(!possible_dirs)
				start = null
				update_icon()
		...
		while (1)
			...
			if(cur == start)
				...
				if(T && !T.density)
					...
					for(var/obj/structure/window/W in T)
						if(W.is_full_window || W.dir == orientation)
							tape_dir = dir
			else if(cur == end)
				...
				if(T && !T.density)
					...
					for(var/obj/structure/window/W in T)
						if(W.is_full_window || W.dir == GLOB.reverse_dir[orientation])
							tape_dir = dir
...
/obj/item/taperoll/afterattack(atom/A, mob/user as mob, proximity)
	...
	if (istype(A, /turf/floor) ||istype(A, /turf/floor))
		var/turf/F = A
