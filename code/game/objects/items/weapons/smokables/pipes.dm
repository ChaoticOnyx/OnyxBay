...
/obj/item/clothing/mask/smokable/pipe/generate_lighting_message(obj/tool, mob/holder)
	...
	if(istype(tool, /obj/item/weapon/flame/candle))
		return SPAN("notice", "[holder] lights \his [name] with a hot wax from \a [tool].")
	if(istype(tool, /obj/item/weapon/weldingtool))
		return SPAN("notice", "[holder] recklessly \his [name] with \a [tool].")
	if(istype(tool, /obj/item/weapon/reagent_containers/glass/rag))
		return SPAN("warning", "[holder] puts a piece of \a [tool] into \a [name] to light it up.")
	if(istype(tool, /obj/item/clothing/mask/smokable/cigarette))
		return SPAN("notice", "[holder] dips \his [tool.name] into \a [name] to light it up.")
