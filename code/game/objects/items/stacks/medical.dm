	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_salved())
			...
			if(user.zone_sel.selecting == BP_MOUTH)
				...
				user.visible_message(SPAN_NOTICE("[user] covers wounds on [M]'s [affecting.name] with protein-renaturating gel."), \
			                         SPAN_NOTICE("You cover wounds on [M]'s [affecting.name] with protein-renaturating gel."))
