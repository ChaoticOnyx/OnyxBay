...
/obj/item/device/pda/proc/new_info(beep_silent, message_tone, reception_message)
	...
	if(loc && isliving(loc))
		...
		if(L.mind && L.mind.syndicate_awareness == SUSPICIOUSLY_AWARED)
			reception_message = highlight_words_with_suffix(reception_message, syndicate_code_phrase)
			//reception_message = highlight_words_with_suffix(reception_message, syndicate_code_response, rose) // <-- Uncomment this to highlight responses too, with different color
