...
/obj/proc/user_buckle_mob(mob/living/M, mob/user)
	if(!user.Adjacent(M) || user.restrained() || user.incapacitated(INCAPACITATION_ALL) || user.stat || istype(user, /mob/living/silicon/pai) || istype(user, /mob/living/simple_animal))
		return 0
	if(M == buckled_mob)
		return 0
