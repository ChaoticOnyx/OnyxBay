...
/obj/item/weapon/hand_labeler/attack_self(mob/user as mob)
   ...
   if(!str || !length(str))
      ...
      return
   if(length(str) > 10)
      usr << "\red Text too long." //this needs to be made bigger at some point
      return
...
/obj/item/weapon/label/afterattack(atom/A, mob/user as mob)
   if(!backing)
      ...
      if(!label || !length(label))
         ...
         return
      if(length(A.name) + length(label) > 25) //this needs to be made bigger too. maybe number of labels instead of a fixed length
         user << "\red Label too big."
         return
      for(var/i = 1, i < A.labels.len, i++)
         if(label == A.labels[i])
            user << "\red The item already has that label!"
            return
...
/obj/item/weapon/label/attack_self(mob/user as mob)
   if(backing)
      backing = 0
