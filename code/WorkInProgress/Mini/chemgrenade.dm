...
/obj/item/device/chemgrenade/attackby(obj/item/item, mob/user)
	if(active)
		return ..()
...
/obj/item/device/chemgrenade/attack_self(mob/user as mob)
	if(active)
...
/obj/item/device/chemgrenade/Topic(href, href_list)
	..()
...
/obj/item/device/chemgrenade/receive_signal(signal)
	if(!active)
		return	//cant go off before it gets primed
	explode()

/obj/item/device/chemgrenade/process()

/obj/item/device/chemgrenade/HasProximity(atom/movable/AM as mob|obj)
...
/obj/item/device/chemgrenade/proc/explode()
	if(exploding) return
	exploding = 1
...
/obj/item/device/chemgrenade/proc/c_state()
	return
