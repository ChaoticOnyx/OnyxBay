...
/datum/camerachunk/proc/update()
	...
	for(var/turf/t in dimRemoved)
		...
		if(!(t in visibleTurfs))
			if(!t.obscured)
				t.obscured = image('code/WorkInProgress/Uristqwerty/cameravis.dmi', t, "black", 15)
	...
	for(var/turf/t in dimAdded)
		if(!(t in visibleTurfs))
			if(!t.dim)
				t.dim = image('code/WorkInProgress/Uristqwerty/cameravis.dmi', t, "dim", 15)
				t.mouse_opacity = 0
...
/datum/camerachunk/New(loc, x, y, z)
	...
	for(var/turf/t in dimTurfs)
		if(!(t in visibleTurfs))
			if(!t.dim)
				t.dim = image('code/WorkInProgress/Uristqwerty/cameravis.dmi', t, "dim", TURF_LAYER)
