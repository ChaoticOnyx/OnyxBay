...
function run_code_tests {
    ...
    run_test "check changelog example unchanged" "md5sum -c - <<< '9ef52338b5195fe0e3816567df32ac2d *html/changelogs/example.yml'"
...
function run_byond_tests {
    ...
    run_test_ci "check globals build" "python3 tools/GenerateGlobalVarAccess/gen_globals.py onyxbay.dme code/_helpers/global_access.dm"
    run_test "check globals unchanged" "md5sum -c - <<< '924e0c573dae1c0128a042f78b9fb7b4 *code/_helpers/global_access.dm'"
    run_test "build map unit tests" "scripts/dm.sh -DUNIT_TEST -M$MAP_PATH onyxbay.dme"
