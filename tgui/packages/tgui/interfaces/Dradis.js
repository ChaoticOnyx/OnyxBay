...
import { Button, Section, Knob, Map } from "../components";
...
export const DradisContent = (props, context) => {
      ...
      <li key={ship.id}>
        {!!ship.name && (
          <Button
            ...
            content=""
            onClick={() => act("hail", { target: ship.id })}
          >
  ...
  const DradisMap = (ships, zoom_factor, width_mod, focus_x, focus_y) => {
    ...
    let rangeStyle =
      "left:" +
      focus_x * scale_factor +
      "px;bottom:" +
      focus_y * scale_factor +
      "px; width:" +
      width_mod * multiplier +
      "px; height:" +
      width_mod * multiplier +
      "px;margin-bottom:" +
      -1 * ((width_mod * multiplier) / 2) +
      "px;margin-left:" +
      -1 * ((width_mod * multiplier) / 2) +
      "px;";
  ...
  return (
    <Section
      ...
      buttons={
        <>
          <Button icon="search-plus" onClick={() => act("zoomin")} />
          <Button icon="search-minus" onClick={() => act("zoomout")} />
          {!!data.can_use_radar && (
            <>
              <Button
                content="Radar Pulse"
...
export const Dradis = (props, context) => {
  ...
  return (
    <Window theme="hackerman" width={700} height={750}>
