...
import { Button, Icon, LabeledList, NoticeBox, Section, Tabs, Stack} from "../components";
...
const buildingsPage = (props: any, context: any) => {
  ...
  return (
    <Flex direction="column">
      <Flex.Item>{navPanel(props, context)}</Flex.Item>
      <Flex.Item mt="0.5rem">
        {buildings?.map((c, i) => buildingCard(c, context, c))}
      </Flex.Item>
    </Flex>
  );
};

const buildingCard = (props: Building, context: any, key: any) => {
  const { data, act } = useBackend<InputData>(context);
  return (
    <Flex key={key} class="Card" direction="column">
        <Stack.Item>
          ...
          buttons={
            <Stack>
              <Stack.Item>
                <Button
                content="Select"
                onClick={() => act('select_building', {building_type: props.type})}
                />
              </Stack.Item>
            </Stack>
          }>
