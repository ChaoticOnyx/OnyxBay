...
export const formatTime = (val, formatType) => {
  ...
  switch (formatType) {
    case "short": {
      const hoursTruncated = hours > 0 ? `${hours}h` : "";
      const minutesTruncated = minutes > 0 ? `${minutes}m` : "";
      const secondsTruncated = seconds > 0 ? `${seconds}s` : "";
      return `${hoursTruncated}${minutesTruncated}${secondsTruncated}`;
    }
    default: {
